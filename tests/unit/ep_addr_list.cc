...
static void insert_addresses(nccl_ofi_ep_addr_list_t *ep_addr_list, size_t num_addr, int ep_num) {
  for (size_t i = 0; i < num_addr; ++i) {
    ...
    int ret = nccl_ofi_ep_addr_list_get(ep_addr_list, &i, sizeof(i), &ep);
    if (ret != 0) {
    ...
    if (i == 0) {
      ...
      ret = nccl_ofi_ep_addr_list_insert(ep_addr_list, (nccl_net_ofi_ep_t *)(uintptr_t)(ep_num), &i, sizeof(i));
      if (ret != 0) {
...
int main(int argc, char *argv[]) {
  ...
  nccl_ofi_ep_addr_list_t *ep_addr_list = nccl_ofi_ep_addr_list_init(128);
  if (!ep_addr_list) {
