...
int main(int argc, char *argv[]) {
  ...
  for (int rounds = 0; rounds < 4; rounds++) {
    /** Test insert new **/
    for (uint16_t i = 0; i < max_inprogress; ++i) {
      if (nccl_ofi_msgbuff_insert(msgbuff, (msg_seq_num + i) % field_size, &buff_store[i], type, &stat) != NCCL_OFI_MSGBUFF_SUCCESS) {
    ...
    /** Test retrieve **/
    for (uint16_t i = 0; i < max_inprogress; ++i) {
      if (nccl_ofi_msgbuff_retrieve(msgbuff, (msg_seq_num + i) % field_size, (void **)&result, &type, &stat) != NCCL_OFI_MSGBUFF_SUCCESS) {
    ...
    if (nccl_ofi_msgbuff_retrieve(msgbuff, (msg_seq_num + max_inprogress) % field_size, (void **)&result, &type, &stat) != NCCL_OFI_MSGBUFF_INVALID_IDX ||
        stat != NCCL_OFI_MSGBUFF_NOTSTARTED) {
      NCCL_OFI_WARN("nccl_ofi_msgbuff_retrieve did not return notstarted");
      return 1;
    }

    if (nccl_ofi_msgbuff_retrieve(msgbuff, last_completed, (void **)&result, &type, &stat) != NCCL_OFI_MSGBUFF_INVALID_IDX ||
    ...
    /** Test complete **/
    for (uint16_t i = 0; i < max_inprogress; ++i) {
      if (nccl_ofi_msgbuff_complete(msgbuff, (msg_seq_num + i) % field_size, &stat) != NCCL_OFI_MSGBUFF_SUCCESS) {
        NCCL_OFI_WARN("nccl_ofi_msgbuff_complete failed");
        return 1;
      }
    }

    if (nccl_ofi_msgbuff_complete(msgbuff, (msg_seq_num + max_inprogress) % field_size, &stat) != NCCL_OFI_MSGBUFF_INVALID_IDX ||
        stat != NCCL_OFI_MSGBUFF_NOTSTARTED) {
      NCCL_OFI_WARN("nccl_ofi_msgbuff_complete did not return notstarted");
      return 1;
    }

    if (nccl_ofi_msgbuff_complete(msgbuff, msg_seq_num, &stat) != NCCL_OFI_MSGBUFF_INVALID_IDX || stat != NCCL_OFI_MSGBUFF_COMPLETED) {
