...
static inline int verify_xfer_info(nccl_net_ofi_xfer_info_t *xfer, nccl_net_ofi_xfer_info_t *ref_xfer, int xfer_id) {
  ...
  if (ret) {
    NCCL_OFI_WARN("Expected rail_xfer_infos[%i] ={rail_id = %i, offset = %zu, msg_size = %zu}, but got {rail_id = %i, offset = %zu, msg_size = %zu}", xfer_id,
                  ref_xfer->rail_id, ref_xfer->offset, ref_xfer->msg_size, xfer->rail_id, xfer->offset, xfer->msg_size);
  }
  return ret;
}
...
static inline int verify_schedule(nccl_net_ofi_schedule_t *schedule, nccl_net_ofi_schedule_t *ref_schedule) {
  ...
  if (schedule->num_xfer_infos != ref_schedule->num_xfer_infos) {
    NCCL_OFI_WARN("Wrong number of xfer infos. Expected %zu, but got %zu", ref_schedule->num_xfer_infos, schedule->num_xfer_infos);
    return 1;
  }

  for (size_t info_id = 0; info_id != schedule->num_xfer_infos; ++info_id) {
    ret |= verify_xfer_info(&schedule->rail_xfer_infos[info_id], &ref_schedule->rail_xfer_infos[info_id], info_id);
  }
...
static inline int test_threshold_scheduler() {
  ...
  */
  ...
  size_t offsets_1[6][1] = {{0}, {0}, {0}, {0}, {0}, {0}}; /* Offset remains 0 in round robin */
  for (int iter = 0; iter < 6; iter++) {
    ret = test_multiplexer(scheduler, num_rails, msg_sizes_1[iter], num_stripes, rail_ids_1[iter], offsets_1[iter], msg_size_per_stripe_1[iter]);
    if (ret) {
      NCCL_OFI_WARN("Verification failed");
      return ret;
    }
  ...
  size_t offsets_2[6][2] = {{0, stripe_size[0]}, {0, stripe_size[1]}, {0, stripe_size[2]}, {0, stripe_size[3]}, {0, stripe_size[4]}, {0, stripe_size[5]}};
  size_t msg_size_per_stripe_2[6][2] = {{stripe_size[0], remaining_stripe_size[0]}, {stripe_size[1], remaining_stripe_size[1]},
                                        {stripe_size[2], remaining_stripe_size[2]}, {stripe_size[3], remaining_stripe_size[3]},
                                        {stripe_size[4], remaining_stripe_size[4]}, {stripe_size[5], remaining_stripe_size[5]}};
  for (int iter = 0; iter < 6; iter++) {
    ret = test_multiplexer(scheduler, num_rails, msg_sizes_2[iter], num_stripes, rail_ids_2[iter], offsets_2[iter], msg_size_per_stripe_2[iter]);
    if (ret) {
      NCCL_OFI_WARN("Verification failed");
      return ret;
    }
  ...
  size_t offsets_3[6][2] = {{0, (stripe_size[0] * 2) / 2}, {0, (stripe_size[1] * 2) / 2}, {0, (stripe_size[2] * 2) / 2},
                            {0, (stripe_size[3] * 2) / 2}, {0, (stripe_size[4] * 2) / 2}, {0, (stripe_size[5] * 2) / 2}};
  size_t msg_size_per_stripe_3[6][2] = {{(stripe_size[0] * 2) / 2, remaining_stripe_size[0]}, {(stripe_size[1] * 2) / 2, remaining_stripe_size[1]},
                                        {(stripe_size[2] * 2) / 2, remaining_stripe_size[2]}, {(stripe_size[3] * 2) / 2, remaining_stripe_size[3]},
                                        {(stripe_size[4] * 2) / 2, remaining_stripe_size[4]}, {(stripe_size[5] * 2) / 2, remaining_stripe_size[5]}};

  for (int iter = 0; iter < 6; iter++) {
    ret = test_multiplexer(scheduler, num_rails, msg_sizes_3[iter], num_stripes, rail_ids_3[iter], offsets_3[iter], msg_size_per_stripe_3[iter]);
    if (ret) {
      NCCL_OFI_WARN("Verification failed");
      return ret;
    }
  ...
  size_t offsets_4[6][4] = {{0, stripe_size[0], stripe_size[0] * 2, stripe_size[0] * 3}, {0, stripe_size[1], stripe_size[1] * 2, stripe_size[1] * 3},
                            {0, stripe_size[2], stripe_size[2] * 2, stripe_size[2] * 3}, {0, stripe_size[3], stripe_size[3] * 2, stripe_size[3] * 3},
                            {0, stripe_size[4], stripe_size[4] * 2, stripe_size[4] * 3}, {0, stripe_size[5], stripe_size[5] * 2, stripe_size[5] * 3}};
  size_t msg_size_per_stripe_4[6][4] = {
      {stripe_size[0], stripe_size[0], stripe_size[0], remaining_stripe_size[0]}, {stripe_size[1], stripe_size[1], stripe_size[1], remaining_stripe_size[1]},
      {stripe_size[2], stripe_size[2], stripe_size[2], remaining_stripe_size[2]}, {stripe_size[3], stripe_size[3], stripe_size[3], remaining_stripe_size[3]},
      {stripe_size[4], stripe_size[4], stripe_size[4], remaining_stripe_size[4]}, {stripe_size[5], stripe_size[5], stripe_size[5], remaining_stripe_size[5]}};

  for (int iter = 0; iter < 6; iter++) {
    ret = test_multiplexer(scheduler, num_rails, msg_sizes_4[iter], num_stripes, rail_ids_4[iter], offsets_4[iter], msg_size_per_stripe_4[iter]);
    if (ret) {
      NCCL_OFI_WARN("Verification failed");
      return ret;
    }
