...
int main(int argc, char *argv[]) {
  ...
  for (int dev_idx = 0; dev_idx < ndev; dev_idx++) {
    ...
    for (int idx = 0; idx < NUM_REQUESTS; idx++) {
      ...
      OFINCCLCHECKGOTO(extNet->regMr((void *)sComm_next, (void *)send_buf[idx], SEND_SIZE, buffer_type, &send_mhandle[idx]), res, exit);
      NCCL_OFI_TRACE(NCCL_NET, "Successfully registered send memory for request %d of rank %d", idx, rank);

      while (send_req[idx] == NULL) {
        OFINCCLCHECKGOTO(extNet->isend((void *)sComm_next, (void *)send_buf[idx], SEND_SIZE, tag, send_mhandle[idx], (void **)&send_req[idx]), res, exit);
      }
    }
    ...
    for (int idx = 0; idx < NUM_REQUESTS; idx++) {
      ...
      OFINCCLCHECKGOTO(extNet->regMr((void *)rComm, (void *)recv_buf[idx], RECV_SIZE, buffer_type, &recv_mhandle[idx]), res, exit);
      NCCL_OFI_TRACE(NCCL_NET, "Successfully registered receive memory for request %d of rank %d", idx, rank);

      while (recv_req[idx] == NULL) {
        OFINCCLCHECKGOTO(extNet->irecv((void *)rComm, nrecv, (void **)&recv_buf[idx], sizes, tags, &recv_mhandle[idx], (void **)&recv_req[idx]), res, exit);
      }
    }
      ...
      for (int idx = 0; idx < NUM_REQUESTS; idx++) {
        ...
        if (done) {
          ...
          if (buffer_type == NCCL_PTR_CUDA) {
            ...
            OFINCCLCHECKGOTO(extNet->iflush((void *)rComm, nrecv, (void **)&recv_buf[idx], sizes, &recv_mhandle[idx], (void **)&iflush_req), res, exit);
            done = 0;
            if (iflush_req) {
              while (!done) {
