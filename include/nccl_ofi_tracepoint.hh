...
#define NCCL_OFI_TRACE_RECV_SENDRECV(dev, tag, size, request, nccl_req)                                                                                        \
  do {                                                                                                                                                         \
    lttng_ust_tracepoint(nccl_ofi_plugin, Recv, dev, tag, size, request, nccl_req);                                                                            \
  } while (0)

#define NCCL_OFI_TRACE_FLUSH_SENDRECV(request, nccl_req)                                                                                                       \
  do {                                                                                                                                                         \
    lttng_ust_tracepoint(nccl_ofi_plugin, Flush, request, nccl_req);                                                                                           \
  } while (0)

#define NCCL_OFI_TRACE_COMPLETIONS_SENDRECV(dev, request, ctx)                                                                                                 \
  do {                                                                                                                                                         \
...
#define NCCL_OFI_TRACE_SEND_END(request)                                                                                                                       \
  do {                                                                                                                                                         \
    NCCL_OFI_TRACE_SEND_END_NVTX(request);                                                                                                                     \
  } while (0)

#define NCCL_OFI_TRACE_EAGER_SEND_START(dev, rail_id, size, comm, msg_seq_num, request)                                                                        \
  do {                                                                                                                                                         \
...
#define NCCL_OFI_TRACE_EAGER_SEND_COMPLETE(dev, rail_id, comm, msg_seq_num, request)                                                                           \
  do {                                                                                                                                                         \
...
#define NCCL_OFI_TRACE_SEND_CTRL_START(dev, rail_id, comm, req, msg_seq_num)                                                                                   \
  do {                                                                                                                                                         \
...
#define NCCL_OFI_TRACE_SEND_CTRL_END(dev, rail_id, comm, req, msg_seq_num)                                                                                     \
  do {                                                                                                                                                         \
...
#define NCCL_OFI_TRACE_RECV_END(request)                                                                                                                       \
  do {                                                                                                                                                         \
    NCCL_OFI_TRACE_RECV_END_NVTX(request);                                                                                                                     \
  } while (0)

#define NCCL_OFI_TRACE_RECV_SEGMENT_COMPLETE(dev, rail_id, size, request, msg_seq_num)                                                                         \
  do {                                                                                                                                                         \
...
#define NCCL_OFI_TRACE_COMPLETIONS(dev, request, ctx)                                                                                                          \
  do {                                                                                                                                                         \
    lttng_ust_tracepoint(nccl_ofi_plugin, ProcessCompletions, dev, request, ctx);                                                                              \
  } while (0)
