 ...
 * get_device_property() */
enum { MAX_DEV_PROPERTY_LENGTH = 16 };
...
static nccl_ofi_topo_data_vec_t *nccl_ofi_topo_data_vec_create(size_t size) {
  nccl_ofi_topo_data_vec_t *vec = nullptr;
...
static int get_hwloc_pcidev_by_fi_info(hwloc_topology_t topo, struct fi_info *info, hwloc_obj_t *obj) {
  *obj = nullptr;
  ...
  ret_obj = hwloc_get_pcidev_by_busid(topo, attr->domain_id, attr->bus_id, attr->device_id, attr->function_id);
...
static int get_info_for_node(hwloc_obj_t node, struct fi_info *info_list, struct fi_info **ret_info) {
  *ret_info = nullptr;
  ...
  do {
    ...
    if (node_attr->pcidev.domain == attr->domain_id && node_attr->pcidev.bus == attr->bus_id && node_attr->pcidev.dev == attr->device_id &&
...
static int count_nodes_with_accel_or_nic_in_subtree(hwloc_topology_t topo, struct fi_info *info_list, int *count) {
  ...
  if (count != nullptr)
    return count_nodes_with_accel_or_nic_in_subtree(topo, info_list, nullptr);
  else
    return 0;
}
...
static int set_userdata_to_root(hwloc_obj_t node, nccl_ofi_topo_data_iterator_t *data_iter) {
  nccl_ofi_topo_data_t *user_data = nullptr;
...
static int set_user_data(nccl_ofi_topo_t *ofi_topo, struct fi_info *info_list) {
  ...
  while ((obj = hwloc_get_next_pcidev(ofi_topo->topo, obj))) {
    ...
    if (info) {
      ...
      auto *user_data = (nccl_ofi_topo_data_t *)obj->userdata;
...
nccl_ofi_topo_t *nccl_ofi_topo_create(struct fi_info *info_list) {
  ...
  auto *ofi_topo = (nccl_ofi_topo_t *)calloc(1, sizeof(nccl_ofi_topo_t));
  if (!ofi_topo) {
...
static int mark_topo_nodes_with_ofi_info_subtree(nccl_ofi_topo_t *topo) {
  ...
  while ((data = nccl_ofi_get_user_data(&data_iter))) {
    ...
    while (obj) {
      auto *obj_data = (nccl_ofi_topo_data_t *)obj->userdata;
      if (!obj_data) {
...
static int lift_up_ofi_infos(nccl_ofi_topo_t *topo) {
  nccl_ofi_topo_data_t *source_data = nullptr;
  nccl_ofi_topo_data_t *target_data = nullptr;
...
static int create_groups_from_info_list(nccl_ofi_topo_t *topo, struct fi_info **info_list, int num_infos, hwloc_obj_t gpu_group_node, int num_groups) {
   ...
   * consistency
   */
  if (platform_sort_rails != nullptr) {
    ...
    while (info) {
      ...
      if (attr) {
        NCCL_OFI_INFO(NCCL_INIT,
                      ...
                      "%04x:%02x:%02x.%01x",
                      group_idx, info_idx, attr->domain_id, attr->bus_id, attr->device_id, attr->function_id);
      }
      ++info_idx;
      info = info->next;
    }
 ...
 *		non-zero, on error
 */
static int get_device_property(unsigned domain, unsigned bus, unsigned dev, unsigned func, const char *prop_name, char *prop) {
  ...
  if ((path_len = snprintf(nullptr, 0, path_format, domain, bus, dev, func, prop_name)) < 0) {
    NCCL_OFI_WARN("Failed to determine device property path length of property %s. ERROR: %s", prop_name, strerror(errno));
    ret = -errno;
    goto error;
  }
  ...
  if (snprintf(path, path_len + 1, path_format, domain, bus, dev, func, prop_name) < 0) {
    NCCL_OFI_WARN("Failed to create device property path for property %s. ERROR: %s", prop_name, strerror(errno));
    ret = -errno;
    goto error;
  }
  ...
  if ((file = fopen(path, "r")) != nullptr) {
    ...
    if (feof(file) && !rc) {
      ...
      NCCL_OFI_WARN("Failed to read device property file %s. ERROR: %s", path, strerror(errno));
      ret = -errno;
      goto exit;
    }
  } else {
    NCCL_OFI_WARN("Failed to open device property file %s. ERROR: %s", path, strerror(errno));
    ret = -errno;
    goto error;
  }
...
exit:
  if (fclose(file)) {
    NCCL_OFI_WARN("Failed to close device property file %s. ERROR: %s", path, strerror(errno));
    ret = -errno;
  }
error:
  if (path)
...
static int get_pci_device_speed(hwloc_obj_t node, bool is_nic, size_t *speed_idx, size_t *width) {
  ...
  /* Read link speed */
  if ((ret = get_device_property(attr.pcidev.domain, attr.pcidev.bus, attr.pcidev.dev, attr.pcidev.func, speed_name, prop_str))) {
    return ret;
  }
  ...
  *speed_idx = 0;
  ...
  if (is_nic && strncmp(override_speed, prop_str, strlen(override_speed)) == 0) {
    ...
    *speed_idx = fallback_speed_idx;
    NCCL_OFI_INFO(NCCL_INIT, "Override link speed \"%s\" of NIC %04x:%02x:%02x.%01x with speed \"%s\"", prop_str, attr.pcidev.domain, attr.pcidev.bus,
                  attr.pcidev.dev, attr.pcidev.func, pcie_gen[*speed_idx]);
  }
  if (*speed_idx == num_pcie_gens) {
    NCCL_OFI_WARN("Unknown link speed \"%s\" of device %04x:%02x:%02x.%01x", prop_str, attr.pcidev.domain, attr.pcidev.bus, attr.pcidev.dev, attr.pcidev.func);
    return -EINVAL;
  }

  /* Read link width */
  if ((ret = get_device_property(attr.pcidev.domain, attr.pcidev.bus, attr.pcidev.dev, attr.pcidev.func, width_name, prop_str))) {
    return ret;
  }
  ...
  if (is_nic && strncmp(override_width, prop_str, strlen(override_width)) == 0) {
    ...
    *width = fallback_width;
    NCCL_OFI_INFO(NCCL_INIT, "Override link width \"%s\" of NIC %04x:%02x:%02x.%01x with width \"%zu\"", prop_str, attr.pcidev.domain, attr.pcidev.bus,
                  attr.pcidev.dev, attr.pcidev.func, *width);
  } else {
    *width = strtol(prop_str, nullptr, 0);
  }
  if (errno == ERANGE) {
    NCCL_OFI_WARN("Unable to convert link width \"%s\" of device %04x:%02x:%02x.%01x to a "
                  "valid link width. "
                  "Error: %s",
                  prop_str, attr.pcidev.domain, attr.pcidev.bus, attr.pcidev.dev, attr.pcidev.func, strerror(errno));
    return -errno;
  } else if (*width == 0) {
    NCCL_OFI_WARN("Unknown link width \"%s\" of device %04x:%02x:%02x.%01x", prop_str, attr.pcidev.domain, attr.pcidev.bus, attr.pcidev.dev, attr.pcidev.func);
    return -EINVAL;
  }
...
int nccl_ofi_topo_write(nccl_ofi_topo_t *topo, FILE *file) {
  ...
  ret = write_nccl_topo_rec(topo->topo, hwloc_get_root_obj(topo->topo), file, indent, bridge_depth);
  if (ret) {
