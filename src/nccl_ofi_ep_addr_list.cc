...
nccl_ofi_ep_addr_list_t *nccl_ofi_ep_addr_list_init(size_t max_addr_size) {
  ...
  ret_list->ep_pair_list = nullptr;
  ret_list->max_addr_size = max_addr_size;

  if (nccl_net_ofi_mutex_init(&ret_list->mutex, nullptr) != 0) {
...
int nccl_ofi_ep_addr_list_get(nccl_ofi_ep_addr_list_t *ep_list, void *addr_in, size_t addr_size, nccl_net_ofi_ep_t **ep) {
  ...
  if (addr_size > ep_list->max_addr_size) {
    NCCL_OFI_WARN("Address size %zu > max size (%zu)", addr_size, ep_list->max_addr_size);
    ret = -EINVAL;
    return ret;
  }
  ...
  ep_pair_list_elem_t *ep_pair = nullptr;

  nccl_net_ofi_ep_t *ret_ep = nullptr;

  DL_FOREACH(ep_list->ep_pair_list, ep_pair) {
    hashed_addr_t *found_handle = nullptr;
    HASH_FIND(hh, ep_pair->addr_set, addr_padded, ep_list->max_addr_size, found_handle);
    if (found_handle) {
      /* This ep already has a connection to the address, skip to next */
      continue;
    } else {
      /* We found an ep that is not connected to addr, so return it */
      auto *new_addr = (hashed_addr_t *)malloc(sizeof(hashed_addr_t) + ep_list->max_addr_size);
      if (!new_addr) {
      ...
      HASH_ADD(hh, ep_pair->addr_set, addr, ep_list->max_addr_size, new_addr);
      ret_ep = ep_pair->ep;
      goto exit;
    }
  }
...
int nccl_ofi_ep_addr_list_insert(nccl_ofi_ep_addr_list_t *ep_list, nccl_net_ofi_ep_t *ep, void *addr_in, size_t addr_size) {
  int ret = 0;
  ep_pair_list_elem_t *new_pair = nullptr;
...
static void delete_ep_list_entry(nccl_ofi_ep_addr_list_t *ep_list, ep_pair_list_elem_t *elem) {
  hashed_addr_t *e = nullptr, *tmp = nullptr;
...
int nccl_ofi_ep_addr_list_delete(nccl_ofi_ep_addr_list_t *ep_list, nccl_net_ofi_ep_t *ep) {
  ...
  ep_pair_list_elem_t *ep_pair = nullptr, *ep_pair_tmp = nullptr;
...
void nccl_ofi_ep_addr_list_fini(nccl_ofi_ep_addr_list_t *ep_list) {
  ...
  {
    ep_pair_list_elem_t *ep_pair = nullptr, *ep_pair_tmp = nullptr;
    DL_FOREACH_SAFE(ep_list->ep_pair_list, ep_pair, ep_pair_tmp) { delete_ep_list_entry(ep_list, ep_pair); }
  }
