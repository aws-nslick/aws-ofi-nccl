...
nccl_ofi_ep_addr_list_t *nccl_ofi_ep_addr_list_init(size_t max_addr_size) {
  nccl_ofi_ep_addr_list_t *ret_list = (nccl_ofi_ep_addr_list_t *)calloc(1, sizeof(*ret_list));
  if (!ret_list) {
...
int nccl_ofi_ep_addr_list_get(nccl_ofi_ep_addr_list_t *ep_list, void *addr_in, size_t addr_size, nccl_net_ofi_ep_t **ep) {
  ...
  DL_FOREACH(ep_list->ep_pair_list, ep_pair) {
    ...
    if (found_handle) {
      ...
      hashed_addr_t *new_addr = (hashed_addr_t *)malloc(sizeof(hashed_addr_t) + ep_list->max_addr_size);
      if (!new_addr) {
