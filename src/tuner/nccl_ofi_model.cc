...
static float nccl_ofi_tuner_compute_cost(nccl_ofi_tuner_model_params_t *params, nccl_ofi_tuner_model_dims_t *dims, ncclFunc_t func, int algo, int proto,
   ...
   */
  net_lat = (proto == NCCL_PROTO_SIMPLE) ? params->net_lat + ofi_nccl_tuner_net_comp_overhead() : params->net_lat;
  ...
  case ncclFuncAllReduce:
    ...
    case NCCL_ALGO_RING:
      ...
      latency = (num_internode_steps * net_lat) + (num_steps - num_internode_steps) * p2p_lat;
    ...
    case NCCL_ALGO_TREE:
      latency = ((2 * ((dims->num_ranks / dims->num_nodes) - 1) * p2p_lat) + (2 * log2(dims->num_nodes) * net_lat));
...
ncclResult_t model_get_coll_info_internal_v3(nccl_ofi_tuner_context_t *ctx, ncclFunc_t collType, size_t nBytes, int numPipeOps, float **collCostTable,
  ...
  if (model_ctx->platform == NCCL_OFI_TUNER_P5_P5E) {
    if (collType == ncclFuncAllReduce && model_ctx->dims.num_nodes == 16 && model_ctx->dims.num_ranks == 128 && nBytes > 3ULL * 1024ULL * 1024ULL * 1024ULL &&
  ...
  for (algo = 0; algo < NCCL_NUM_ALGORITHMS; algo++) {
    ...
    for (proto = 0; proto < NCCL_NUM_PROTOCOLS; proto++) {
      ...
      cost = nccl_ofi_tuner_compute_cost(model_ctx->model_params, &model_ctx->dims, collType, algo, proto, numPipeOps, nBytes);
      if (cost < 0)
        continue;

      NCCL_OFI_TRACE(NCCL_TUNING, "Model Tuner Computed cost for algo %d proto %d pipe %d: cost %.8f µsecs.", algo, proto, numPipeOps, cost);
      if (cost < lowest) {
        chosen_algo = algo;
        chosen_proto = proto;
        lowest = cost;
...
table_update:
  ...
  NCCL_OFI_INFO(NCCL_TUNING, "Model Tuner Choosing algo %d proto %d with cost %.8f µsecs for coll %d size %ld.", chosen_algo, chosen_proto,
                table[chosen_algo][chosen_proto], collType, nBytes);
...
ncclResult_t model_get_coll_info_internal_v2(nccl_ofi_tuner_context_t *ctx, ncclFunc_t collType, size_t nBytes, int collNetSupport, int nvlsSupport,
  ...
  /* apply p5/p5e platform specific quirk */
  if (model_ctx->platform == NCCL_OFI_TUNER_P5_P5E) {
    if (collType == ncclFuncAllReduce && model_ctx->dims.num_nodes == 16 && model_ctx->dims.num_ranks == 128 && nvlsSupport &&
...
exit:
  NCCL_OFI_INFO(NCCL_TUNING, "Model Tuner Choosing algo %d proto %d with cost %.8f µsecs for coll %d size %ld.", *algorithm, *protocol, lowest, collType,
                nBytes);
  return ncclSuccess;
}
