...
ec2_platform_data *platform_aws_get_platform_entry(const char *platform_type, ec2_platform_data *platform_data_list, size_t platform_data_len) {
  ...
  NCCL_OFI_TRACE(NCCL_NET | NCCL_INIT, "Using platform block %s for instance type %s", (response == NULL) ? "none" : response->name, platform_type);
...
static int validate_rdma_write(fid_ep *ep) {
  ...
  if (ret != 0) {
    ...
    NCCL_OFI_WARN("Unexpected response size when checking FI_OPT_EFA_EMULATED_WRITE.  Expected %lu, got %lu", sizeof(optval), optlen);
    ret = -EINVAL;
    goto exit;
  } else if (optval) {
...
static int configure_nccl_proto() {
  ...
  if (!getenv("NCCL_PROTO")) {
    ...
    NCCL_OFI_WARN("NCCL_PROTO was set to \"LL/LL128\", but the Libfabric endpoint does not support 128 byte in-order aligned stores. This endpoint may corrupt "
...
static int configure_ep_inorder(fid_ep *ep, int optname, const char *optname_name, bool *have_ordering) {
  ...
  *have_ordering = false;
  ...
  NCCL_OFI_TRACE(NCCL_INIT, "fi_setopt(%s) ordering result %s, error code %d", optname_name, have_ordering ? "yes" : "no", ret);
#else
  *have_ordering = false;
#endif
  return 0;
 ...
 *
 * @return	0, when we are successfully able to update NCCL topology or
 * 		   if we find no match
 * 		error, on failure
 */
int platform_init(const char **provider_filter) {
  ...
  if (getenv("NCCL_TOPO_FILE")) {
    NCCL_OFI_INFO(NCCL_INIT | NCCL_NET, "Running on %s platform, NCCL_TOPO_FILE environment variable is already set to %s", nccl_net_ofi_get_product_name(),
    ...
    if (ret < 0 || (size_t)ret >= sizeof(topology_path)) {
      NCCL_OFI_WARN("Error occurred while forming the complete topology XML file path. RC: %d, Buffer Size: %d, XML dir: %s, Topology file: %s", ret, PATH_MAX,
                    XML_DIR, platform_data->topology);
      ret = -ENOMEM;
      goto exit;
    }

    NCCL_OFI_INFO(NCCL_INIT | NCCL_NET, "Running on %s platform, Setting NCCL_TOPO_FILE environment variable to %s", nccl_net_ofi_get_product_name(),
                  topology_path);
...
int platform_config_endpoint(fi_info *info, fid_ep *endpoint) {
  ...
  if (0 == strcasecmp("SENDRECV", nccl_ofi_selected_protocol)) {
    ...
    NCCL_OFI_WARN("unknown transport %s", nccl_ofi_selected_protocol);
    ret = -EINVAL;
    goto exit;
  }
