 ...
 * 		error, others
 */
static int sendrecv_recv_conn_post(nccl_net_ofi_sendrecv_listen_comm_t *l_comm, nccl_net_ofi_sendrecv_ep_t *ep, void *buffer, size_t size,
  ...
  rc = fi_trecv(l_comm->local_ep, buffer, size, NULL, FI_ADDR_UNSPEC, l_comm->tag | (ep->max_tag + 1), 0, &req->ctx);
  if (rc == -FI_EAGAIN) {
    /*
    ...
    NCCL_OFI_WARN("Unable to post a buffer for receving connections for dev %d. RC: %zd, ERROR: %s", dev_id, rc, fi_strerror(-rc));
...
static int sendrecv_listen_comm_accept(nccl_net_ofi_listen_comm_t *listen_comm, nccl_net_ofi_recv_comm_t **recv_comm) {
  ...
  *recv_comm = NULL;
  ...
  case COMM_RECV_CONN:
    ...
    ret = sendrecv_recv_conn_post(l_comm, ep, conn_info, sizeof(nccl_ofi_connection_info_t), req);
    if (ret == -FI_EAGAIN) {
