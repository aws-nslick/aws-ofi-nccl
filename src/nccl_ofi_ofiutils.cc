 ...
 *		false, otherwise
 */
static bool match_prov_info(char *name, uint32_t addr_format, uint64_t mem_tag_format, uint64_t expected_mem_tag_format) {
...
int nccl_ofi_ofiutils_init_connection(fi_info *info, fid_domain *domain, fid_ep **ep, fid_av **av, fid_cq **cq) {
...
#if HAVE_DECL_FI_OPT_SHARED_MEMORY_PERMITTED
  {
    ...
    if (ret == -FI_EOPNOTSUPP || ret == -FI_ENOPROTOOPT) {
      ...
      NCCL_OFI_WARN("Disabling shared memory failed: %s", fi_strerror(-ret));
      goto error;
    }
  }
#endif
  ...
  if (FI_VERSION_GE(info->fabric_attr->api_version, FI_VERSION(1, 18)) && support_gdr != GDR_UNSUPPORTED) {
    ...
    if (ret == -FI_EOPNOTSUPP || ret == -FI_ENOPROTOOPT) {
      ...
      NCCL_OFI_WARN("Failed to set FI_OPT_CUDA_API_PERMITTED. RC: %d, ERROR: %s", ret, fi_strerror(-ret));
      goto error;
    }
#elif HAVE_NEURON
    /*
...
int nccl_ofi_mr_keys_need_own_key(fi_info *provider, bool *provide_own_mr_key) {
  if (!(provider->caps & FI_RMA)) {
    ...
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Provider %s only configured for local registration.", provider->fabric_attr->prov_name);
    *provide_own_mr_key = false;
  } else if (provider->domain_attr->mr_mode & FI_MR_PROV_KEY) {
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Provider %s selects memory registration keys", provider->fabric_attr->prov_name);
    *provide_own_mr_key = false;
  } else {
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Provider %s does not select memory registration keys", provider->fabric_attr->prov_name);
    *provide_own_mr_key = true;

    if (provider->domain_attr->mr_key_size < ofi_nccl_mr_key_size()) {
      NCCL_OFI_WARN("Provider %s supports MR key size of %zu, but %zu was requested", provider->fabric_attr->prov_name, provider->domain_attr->mr_key_size,
