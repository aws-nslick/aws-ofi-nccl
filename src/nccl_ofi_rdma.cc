...
#include "aon/detail/transport/rdma/device.hh"
#include "aon/detail/transport/rdma/close_message.hh"
...
#include "aon/detail/transport/rdma/domain.hh"
#include "aon/detail/transport/rdma/endpoint.hh"
#include "aon/detail/transport/rdma/endpoint_name.hh"
#include "aon/detail/transport/rdma/listen_communicator.hh"
#include "aon/detail/transport/rdma/message_type.hh"
...
#include "aon/detail/transport/rdma/send_communicator.hh"
...
static nccl_ofi_deque_t *s_comm_cleanup_list = nullptr;
static nccl_ofi_deque_t *r_comm_cleanup_list = nullptr;
...
/* Maximum size of RMA write operations */
...
static int repost_rx_buff(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_rdma_req_t *rx_buff_req);

static int post_rx_buffer(nccl_net_ofi_rdma_req_t *req, nccl_net_ofi_ep_rail_t *ep_rail, bool set_fi_more);

static nccl_net_ofi_rdma_req_t *allocate_req(nccl_ofi_freelist_t *fl);

static int free_base_req(uint64_t *num_inflight_reqs, nccl_ofi_freelist_t *nccl_ofi_reqs_fl, nccl_net_ofi_rdma_req_t *req, bool dec_inflight_reqs);

static int check_post_rx_buff_req(nccl_net_ofi_rdma_req_t *rx_buff_req);

static nccl_net_ofi_rdma_domain_t *rdma_endpoint_get_domain(nccl_net_ofi_rdma_ep_t *ep) { return (nccl_net_ofi_rdma_domain_t *)ep->base.domain; }
...
static nccl_net_ofi_rdma_device_t *rdma_domain_get_device(nccl_net_ofi_rdma_domain_t *domain) { return (nccl_net_ofi_rdma_device_t *)domain->base.device; }

static nccl_net_ofi_rdma_plugin_t *rdma_device_get_plugin(nccl_net_ofi_rdma_device_t *device) { return (nccl_net_ofi_rdma_plugin_t *)device->base.plugin; }

static nccl_net_ofi_rdma_ep_t *rdma_req_get_ep(nccl_net_ofi_rdma_req_t *req) { return (nccl_net_ofi_rdma_ep_t *)req->comm->ep; }
 ...
 */
static nccl_net_ofi_comm_t *rdma_device_get_comm(nccl_net_ofi_rdma_device_t *device, uint32_t local_comm_id) {
 ...
 */
static void rdma_device_set_comm(nccl_net_ofi_rdma_device_t *device, uint32_t local_comm_id, nccl_net_ofi_comm_t *comm) {
  assert(local_comm_id < NCCL_OFI_RDMA_MAX_COMMS);
  assert(local_comm_id < device->num_comm_ids);
 ...
 */
static nccl_net_ofi_rdma_listen_comm_t *rdma_device_get_listen_comm(nccl_net_ofi_rdma_device_t *device, uint32_t local_comm_id) {
  auto *l_comm = (nccl_net_ofi_rdma_listen_comm_t *)rdma_device_get_comm(device, local_comm_id);
 ...
 */
static nccl_net_ofi_rdma_send_comm_t *rdma_device_get_send_comm(nccl_net_ofi_rdma_device_t *device, uint32_t local_comm_id) {
  auto *s_comm = (nccl_net_ofi_rdma_send_comm_t *)rdma_device_get_comm(device, local_comm_id);
 ...
 */
static nccl_net_ofi_rdma_recv_comm_t *rdma_device_get_recv_comm(nccl_net_ofi_rdma_device_t *device, uint32_t local_comm_id) {
  auto *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)rdma_device_get_comm(device, local_comm_id);
 ...
 */
static nccl_ofi_rdma_connection_info_t *get_rx_connection_msg(rdma_req_rx_buff_data_t *rx_buff_data) {
 ...
 */
static nccl_net_ofi_rdma_ctrl_msg_t *get_rx_ctrl_msg(rdma_req_rx_buff_data_t *rx_buff_data) {
 ...
 */
static nccl_net_ofi_rdma_close_msg_t *rx_get_close_msg(rdma_req_rx_buff_data_t *rx_buff_data) {
  auto *close_msg = (nccl_net_ofi_rdma_close_msg_t *)rx_buff_data->rx_buff_fl_elem->ptr;
 ...
 */
static nccl_net_ofi_rdma_ctrl_msg_t *rdma_send_ctrl_get_msg(rdma_req_send_ctrl_data_t *send_ctrl_data) {
 ...
 */
static nccl_net_ofi_rdma_close_msg_t *rdma_send_close_get_msg(rdma_req_send_close_data_t *send_close_data) {
 ...
 */
static nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t *rdma_send_comm_get_rail(nccl_net_ofi_rdma_send_comm_t *s_comm, int rail_id) {
  assert(s_comm->rails);
 ...
 */
static nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t *rdma_send_comm_get_control_rail(nccl_net_ofi_rdma_send_comm_t *s_comm, int rail_id) {
 ...
 */
static nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_recv_comm_rail_t *rdma_recv_comm_get_rail(nccl_net_ofi_rdma_recv_comm_t *r_comm, int rail_id) {
  assert(r_comm->rails);
 ...
 */
static nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_recv_comm_rail_t *rdma_recv_comm_get_control_rail(nccl_net_ofi_rdma_recv_comm_t *r_comm, int rail_id) {
  assert(r_comm->control_rails);
  assert(rail_id < r_comm->num_control_rails);
  return &r_comm->control_rails[rail_id];
}

static nccl_net_ofi_rdma_ep_t *rdma_recv_comm_get_ep(nccl_net_ofi_rdma_recv_comm_t *r_comm) { return (nccl_net_ofi_rdma_ep_t *)r_comm->base.base.ep; }

/*
 * @brief Return device rail with index `rail_id`
 */
static nccl_net_ofi_rdma_device_t::nccl_net_ofi_rdma_device_rail_t *rdma_device_get_rail(nccl_net_ofi_rdma_device_t *device, int rail_id) {
  assert(device->device_rails);
  assert(rail_id < device->num_rails);
  return &device->device_rails[rail_id];
}

static nccl_net_ofi_rdma_domain_t::nccl_net_ofi_rdma_domain_rail_t *rdma_domain_get_rail(nccl_net_ofi_rdma_domain_t *domain, int rail_id) {
 ...
 */
static nccl_net_ofi_ep_rail_t *rdma_endpoint_get_rail(nccl_net_ofi_rdma_ep_t *ep, int rail_id) {
  assert(ep->rails);
 ...
 */
static nccl_net_ofi_ep_rail_t *rdma_endpoint_get_control_rail(nccl_net_ofi_rdma_ep_t *ep, int rail_id) {
 ...
 */
static fid_domain *rdma_endpoint_get_ofi_domain(nccl_net_ofi_rdma_ep_t *ep, int rail_id) { return rdma_endpoint_get_rail(ep, rail_id)->domain; }

/*
 * @brief return the domain for the control endpoint and rail.
 */
static fid_domain *rdma_endpoint_get_ofi_control_domain(nccl_net_ofi_rdma_ep_t *ep, int rail_id) { return rdma_endpoint_get_control_rail(ep, rail_id)->domain; }
...
static int write_topo_file(nccl_ofi_topo_t *topo) {
  ...
  if (getenv("NCCL_TOPO_FILE")) {
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "NCCL_TOPO_FILE environment variable is already set to %s", getenv("NCCL_TOPO_FILE"));
    goto exit;
  }
  ...
  if (topo_fd == -1) {
    NCCL_OFI_WARN("Failed to create anonymous topology file. ERROR: %s", strerror(errno));
    ret = -errno;
    goto exit;
  }
  ...
  if (file == nullptr) {
    NCCL_OFI_WARN("Failed to open NCCL topology file using file descriptor. ERROR %s", strerror(errno));
    ret = -errno;
    close(topo_fd);
    goto exit;
  }
  ...
  /* Flush buffered writes to file. We don't close the file here so that
     ...
     in `NCCL_TOPO_FILE`. */
  if (fflush(file) == EOF) {
    NCCL_OFI_WARN("Unable to flush NCCL topology file. ERROR: %s", strerror(errno));
    ret = -errno;
    goto error;
  }
  ...
  if (snprintf(filename, sizeof(filename), "/proc/self/fd/%d", topo_fd) < 0) {
    NCCL_OFI_WARN("Error preparing topo file name");
    ret = -EIO;
    goto error;
  }

  /* Set topology file path environment variable `NCCL_TOPO_FILE` */
  NCCL_OFI_INFO(NCCL_INIT | NCCL_NET, "Setting NCCL_TOPO_FILE environment variable to %s", filename);
 ...
 *		non-zero on error
 */
static int set_mr_req_attr(nccl_ofi_idpool_t *key_pool, int dev_id, nccl_ofi_mr_ckey_ref ckey, uint64_t *flags, int type, fi_mr_attr *mr_attr) {
  int ret = 0;
  ...
  if (nccl_ofi_idpool_active(key_pool)) {
    const int key = nccl_ofi_idpool_allocate_id(key_pool);
    if (key < 0) [[unlikely]] {
...
static int register_rail_mr_buffer(fid_domain *domain, fid_ep *ep, int dev_id, int type, fi_mr_attr *mr_attr, uint64_t flags, fid_mr **mr_handle) {
  int ret = 0;
 ...
 */
static int ofi_info_list_length(fi_info *info_list) {
  int length = 0;
...
static int get_properties(nccl_net_ofi_device_t *base_dev, nccl_ofi_properties_t *props) {
  auto *device = (nccl_net_ofi_rdma_device_t *)base_dev;
  const int dev_id = device->base.dev_id;
  int ret = 0;

  /* Retrieve NIC properties of first rail */
  fi_info *info = device->device_rails[0].info;
  ...
  if (ret == 0) {
    ...
    static_assert(NCCL_OFI_RDMA_COMM_ID_BITS < 31, "NCCL_OFI_RDMA_COMM_ID_BITS must be less than 31 so max_communicators fits in an integer");
 ...
 */
static rdma_req_rx_buff_data_t *get_rx_buff_data(nccl_net_ofi_rdma_req_t *req) {
  assert(req->type == NCCL_OFI_RDMA_RX_BUFF);
  return &req->rx_buff_data;
}

/*
 * @brief	Return write struct of write request
 */
static rdma_req_rma_op_data_t *req_get_rma_op_data(nccl_net_ofi_rdma_req_t *req, nccl_net_ofi_rdma_req_type_t type) {
 ...
 */
static rdma_req_send_data_t *get_send_data(nccl_net_ofi_rdma_req_t *req) {
 ...
 */
static rdma_req_recv_data_t *get_recv_data(nccl_net_ofi_rdma_req_t *req) {
 ...
 */
static rdma_req_send_ctrl_data_t *get_send_ctrl_data(nccl_net_ofi_rdma_req_t *req) {
 ...
 */
static rdma_req_send_close_data_t *req_get_send_close_data(nccl_net_ofi_rdma_req_t *req) {
 ...
 */
static rdma_req_eager_copy_data_t *get_eager_copy_data(nccl_net_ofi_rdma_req_t *req) {
 ...
 */
static rdma_req_recv_segms_data_t *get_recv_segms_data(nccl_net_ofi_rdma_req_t *req) {
 ...
 */
static rdma_req_flush_data_t *get_flush_data(nccl_net_ofi_rdma_req_t *req) {
 ...
 *		The request
 */
static void set_request_state_to_error(nccl_net_ofi_rdma_req_t *req) {
  req->state = NCCL_OFI_RDMA_REQ_ERROR;

  /* Set state of parent requests to error as well */
  if (req->type == NCCL_OFI_RDMA_SEND_CTRL) {
    const rdma_req_send_ctrl_data_t *send_ctrl_data = get_send_ctrl_data(req);
    send_ctrl_data->recv_req->state = NCCL_OFI_RDMA_REQ_ERROR;
  } else if (req->type == NCCL_OFI_RDMA_RECV_SEGMS) {
    const rdma_req_recv_segms_data_t *recv_segms_data = get_recv_segms_data(req);
 ...
 *		non-zero, on error
 */
static int inc_req_completion(nccl_net_ofi_rdma_req_t *req, size_t size, int total_ncompls) {
  const int ret = 0;
  int ncompls = 0;
  ...
  if (ncompls == total_ncompls && (req->state != NCCL_OFI_RDMA_REQ_ERROR)) [[likely]] {
 ...
 *		non-zero, on error
 */
static int set_eager_copy_completed(nccl_net_ofi_rdma_req_t *req) {
  assert(req->type == NCCL_OFI_RDMA_EAGER_COPY);
  int ret = 0;
  const rdma_req_eager_copy_data_t *eager_copy_data = get_eager_copy_data(req);
  nccl_net_ofi_rdma_req_t *recv_req = eager_copy_data->recv_req;
  const rdma_req_recv_data_t *recv_data = get_recv_data(recv_req);
  ...
  const rdma_req_rx_buff_data_t *rx_buff_data = get_rx_buff_data(eager_copy_data->eager_rx_buff_req);
  const size_t size = rx_buff_data->recv_len;
 ...
 *		non-zero, on error
 */
static int set_send_ctrl_completed(nccl_net_ofi_rdma_req_t *req) {
  assert(req->type == NCCL_OFI_RDMA_SEND_CTRL);
  const rdma_req_send_ctrl_data_t *send_ctrl_data = get_send_ctrl_data(req);
  ...
  auto *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)req->comm;
 ...
 *		non-zero, on error
 */
static int inc_recv_seg_completion(nccl_net_ofi_rdma_req_t *req, size_t size, int total_nsegms) {
  assert(req->type == NCCL_OFI_RDMA_RECV_SEGMS);
  int ret = 0;
  bool segms_received = false;
...
static int update_send_data_from_remote(nccl_net_ofi_rdma_send_comm_t *s_comm, nccl_net_ofi_rdma_req_t *rx_buff_req, nccl_net_ofi_rdma_req_t *req) {
  auto *ep = (nccl_net_ofi_rdma_ep_t *)s_comm->base.base.ep;
  assert(ep != NULL);

  const nccl_net_ofi_rdma_device_t *device = rdma_endpoint_get_device(ep);
  ...
  const nccl_net_ofi_rdma_ctrl_msg_t *ctrl_msg = get_rx_ctrl_msg(rx_buff_data);
  ...
  if (send_data->remote_len < send_data->buff_len) {
    NCCL_OFI_TRACE(NCCL_NET, "Remote recv buffer (%zu) smaller than send buffer (%zu)", send_data->remote_len, send_data->buff_len);
  ...
  if (send_data->schedule == nullptr) [[unlikely]] {
    return -EINVAL;
  }
  ...
  send_data->wdata = GET_RDMA_WRITE_IMM_DATA(s_comm->remote_comm_id, req->msg_seq_num, send_data->schedule->num_xfer_infos);
 ...
 */
static int check_post_rx_buffers_rail(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_ep_rail_t *rail) {
...
static int repost_rx_buff(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_rdma_req_t *rx_buff_req) {
  ...
  const rdma_req_rx_buff_data_t *rx_buff_data = get_rx_buff_data(rx_buff_req);
 ...
 */
static int decrease_rx_buff_cnt(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_ep_rail_t *rail) {
 ...
 */
static int handle_ctrl_recv(nccl_net_ofi_rdma_send_comm_t *s_comm, uint16_t msg_seq_num, nccl_net_ofi_rdma_req_t *rx_buff_req) {
  int ret = 0;

  nccl_ofi_msgbuff_status_t stat;
  auto *ep = (nccl_net_ofi_rdma_ep_t *)s_comm->base.base.ep;
  nccl_ofi_msgbuff_result_t mb_res = nccl_ofi_msgbuff_insert(s_comm->msgbuff, msg_seq_num, rx_buff_req, NCCL_OFI_MSGBUFF_BUFF, &stat);
  ...
  if (mb_res != NCCL_OFI_MSGBUFF_INVALID_IDX || stat != NCCL_OFI_MSGBUFF_INPROGRESS) [[unlikely]] {
  ...
  if (mb_res != NCCL_OFI_MSGBUFF_SUCCESS || type != NCCL_OFI_MSGBUFF_REQ) [[unlikely]] {
    NCCL_OFI_WARN("Invalid message retrieval result for msg %hu", msg_seq_num);
    return -EINVAL;
  }

  auto *req = (nccl_net_ofi_rdma_req_t *)elem;
  ...
  if (!send_data->eager) {
    ...
    nccl_net_ofi_mutex_lock(&req->req_lock);
    if (send_data->remote_len < send_data->buff_len) {
      NCCL_OFI_TRACE(NCCL_NET, "Remote recv buffer (%zu) smaller than send buffer (%zu) in eager send", send_data->remote_len, send_data->buff_len);
      req->size = send_data->remote_len;
      send_data->buff_len = send_data->remote_len;
    }
    nccl_net_ofi_mutex_unlock(&req->req_lock);
...
static int free_eager_copy_req(nccl_net_ofi_rdma_req_t *req, bool dec_inflight_reqs) {
  ...
  return free_base_req(&r_comm->num_inflight_reqs, r_comm->nccl_ofi_reqs_fl, req, dec_inflight_reqs);
}

static int alloc_eager_copy_req(nccl_net_ofi_rdma_req_t *recv_req, nccl_net_ofi_rdma_recv_comm_t *r_comm, nccl_net_ofi_rdma_req_t *rx_buff_req) {
  nccl_net_ofi_rdma_req_t *eager_copy_req = allocate_req(r_comm->nccl_ofi_reqs_fl);
  if (eager_copy_req == nullptr) {
 ...
 */
static int handle_eager_recv(nccl_net_ofi_rdma_recv_comm_t *r_comm, uint16_t msg_seq_num, nccl_net_ofi_rdma_req_t *rx_buff_req) {
  int ret = 0;
  auto *ep = (nccl_net_ofi_rdma_ep_t *)r_comm->base.base.ep;
  ...
  nccl_ofi_msgbuff_status_t stat;
  nccl_ofi_msgbuff_result_t mb_res = nccl_ofi_msgbuff_insert(r_comm->msgbuff, msg_seq_num, rx_buff_req, NCCL_OFI_MSGBUFF_BUFF, &stat);
  ...
  if (mb_res != NCCL_OFI_MSGBUFF_INVALID_IDX) [[unlikely]] {
    NCCL_OFI_WARN("Unexpected message insert result (%d) (eager recv)", (int)mb_res);
    return -EINVAL;
  }

  if (stat != NCCL_OFI_MSGBUFF_INPROGRESS) [[unlikely]] {
  ...
  auto *recv_req = (nccl_net_ofi_rdma_req_t *)elem;
  const rdma_req_recv_data_t *recv_data = get_recv_data(recv_req);
...
static int handle_close_msg_recv(nccl_net_ofi_rdma_req_t *rx_buff_req) {
  ...
  const nccl_net_ofi_rdma_close_msg_t *close_msg = rx_get_close_msg(rx_buff_data);
 ...
 */
static int handle_rx_buff_recv(nccl_net_ofi_rdma_device_t *device, int rail_id, fi_cq_data_entry *cq_entry, nccl_net_ofi_rdma_req_t *rx_buff_req, bool eager) {
  int ret = 0;
  rdma_req_rx_buff_data_t *rx_buff_data = nullptr;
  const nccl_ofi_rdma_connection_info_t *conn_msg = nullptr;
  const nccl_ofi_rdma_connection_info_t *conn_resp_msg = nullptr;
  const nccl_net_ofi_rdma_ctrl_msg_t *ctrl_msg = nullptr;
  ...
  if (rx_buff_req == nullptr) [[unlikely]] {
    NCCL_OFI_WARN("RECV event had NULL ctx!");
    return -EINVAL;
  }

  if (rx_buff_req->type != NCCL_OFI_RDMA_RX_BUFF) [[unlikely]] {
   ...
   * type from there. */
  const nccl_ofi_rdma_msg_type_t msg_type = eager ? (nccl_ofi_rdma_msg_type_t)NCCL_OFI_RDMA_MSG_EAGER : get_rx_ctrl_msg(rx_buff_data)->type;
  ...
  case NCCL_OFI_RDMA_MSG_EAGER:
    ...
    NCCL_OFI_TRACE_EAGER_RECV(r_comm->base.base.dev_id, rail_id, r_comm, GET_SEQ_NUM_FROM_IMM(cq_entry->data));
 ...
 */
static nccl_net_ofi_rdma_req_t *get_req_from_imm_data(nccl_net_ofi_rdma_device_t *device, uint64_t data) {
  const uint32_t comm_id = GET_COMM_ID_FROM_IMM(data);
  const nccl_net_ofi_rdma_recv_comm_t *r_comm = rdma_device_get_recv_comm(device, comm_id);

  const uint16_t msg_seq_num = GET_SEQ_NUM_FROM_IMM(data);
  void *elem = nullptr;
  nccl_ofi_msgbuff_elemtype_t type;
  nccl_ofi_msgbuff_status_t stat;

  const nccl_ofi_msgbuff_result_t mb_res = nccl_ofi_msgbuff_retrieve(r_comm->msgbuff, msg_seq_num, &elem, &type, &stat);
  if (mb_res != NCCL_OFI_MSGBUFF_SUCCESS) [[unlikely]] {
  ...
  if (type != NCCL_OFI_MSGBUFF_REQ) [[unlikely]] {
 ...
 */
static int handle_write_comp(fi_cq_data_entry *cq_entry, nccl_net_ofi_rdma_device_t *device, int rail_id) {
  int ret = 0;
  ...
  const rdma_req_recv_data_t *recv_data = get_recv_data(req);
  nccl_net_ofi_rdma_req_t *recv_segms_req = recv_data->recv_segms_req;

  const uint64_t total_segms = GET_NUM_SEG_FROM_IMM(cq_entry->data);
 ...
 */
static int handle_flush_comp(nccl_net_ofi_rdma_req_t *req) {
  int ret = 0;
  const rdma_req_flush_data_t *flush_data = get_flush_data(req);
...
static const char *nccl_net_ofi_req_str(nccl_net_ofi_rdma_req_t *req) {
  static char buf[256];
  snprintf(buf, sizeof(buf), "{ dev: %d, size: %zu, state: %s, type: %s }", req->dev_id, req->size, req_state_str(req->state), req_type_str(req->type));
  return buf;
}
 ...
 *		error, on others
 */
static int process_completions(fi_cq_data_entry *cq_entry, uint64_t num_cqes, nccl_net_ofi_rdma_device_t *device, int rail_id) {
  int ret = 0;
  nccl_net_ofi_rdma_req_t *req = nullptr;
  uint64_t comp_idx = 0, comp_flags = 0;

  const rdma_req_send_data_t *send_data = nullptr;
  const rdma_req_rma_op_data_t *rma_op_data = nullptr;
  ...
  for (comp_idx = 0; comp_idx < num_cqes; comp_idx++) {
    ...
    if (comp_flags & FI_SEND) {
      ...
      /* Receive completions */
      ret = handle_rx_buff_recv(device, rail_id, &cq_entry[comp_idx], req, comp_flags & FI_REMOTE_CQ_DATA);
 ...
 *		error, on others
 */
static int process_err_completion(nccl_net_ofi_rdma_device_t *device, fid_cq *cq) {
  fi_cq_err_entry err_entry = {};
  nccl_net_ofi_rdma_req_t *req = nullptr;
  int ret = 0;

  ret = fi_cq_readerr(cq, &err_entry, 0);
  if (ret == -FI_EAGAIN) [[unlikely]] {
    /*
    ...
    NCCL_OFI_WARN("Unable to read from fi_cq_readerr. RC: %d. Error: %s", ret, fi_strerror(-ret));
    goto exit;
  }
  ...
  NCCL_OFI_WARN("Request %p completed with error. RC: %d. Error: %d (%s). Completed length: %ld, Request: %s", req, err_entry.err, err_entry.prov_errno,
                fi_cq_strerror(cq, err_entry.prov_errno, err_entry.err_data, NULL, 0), (long)err_entry.len, nccl_net_ofi_req_str(req));
...
static int post_rma_read(nccl_net_ofi_rdma_req_t *req) {
  const rdma_req_rma_op_data_t *rma_op_data = req_get_rma_op_data(req, NCCL_OFI_RDMA_READ);
  auto *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)req->comm;
  const nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_recv_comm_rail_t *comm_rail = rdma_recv_comm_get_rail(r_comm, 0);

  ssize_t rc = 0;
  /* Post RMA read */
  rc = fi_read(comm_rail->local_ep, rma_op_data->buff, rma_op_data->buff_len, rma_op_data->desc, comm_rail->remote_addr, rma_op_data->remote_buff,
               rma_op_data->remote_mr_key, req);

  if ((rc != 0) && (rc != -FI_EAGAIN)) {
    NCCL_OFI_WARN("fi_read failed; RC: %zd, Error: %s", rc, fi_strerror(-rc));
  }
...
static int receive_progress(nccl_net_ofi_rdma_req_t *req, bool add_to_pending) {
  ...
  if (rc == -FI_EAGAIN && add_to_pending) {
    const auto *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)req->comm;
    /* Extract ep */
    const auto *ep = (nccl_net_ofi_rdma_ep_t *)r_comm->base.base.ep;
    /* Place in pending requests queue for next try */
    const int ret = nccl_ofi_deque_insert_back(ep->pending_reqs_queue, &req->pending_reqs_elem);
    if (ret != 0) {
      NCCL_OFI_WARN("Failed to nccl_ofi_deque_insert_back: %d", ret);
      return ret;
    } else {
...
static int ofi_process_cq_rail(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_ep_rail_t *rail) {
  fi_cq_data_entry cqe_buffers[cq_read_count];
  ssize_t rc = 0;
  int ret = 0;
  ...
  while (true) {
    ...
    if (rc > 0) {
      ...
      NCCL_OFI_WARN("Unable to retrieve completion queue entries. RC: %zd, ERROR: %s", rc, fi_strerror(-rc));
      ret = -EINVAL;
      goto exit;
    }
  }
 ...
 *		This also updates several request fields like size, status, etc
 *
 * @return	0, on success
 *		error, on others
 */
static int ofi_process_cq(nccl_net_ofi_rdma_ep_t *ep) {
  ...
  if (ret != 0 && ret != -FI_EAGAIN) [[unlikely]] {
 ...
 */
static void zero_nccl_ofi_req(nccl_net_ofi_rdma_req_t *req) {
  req->comm = nullptr;
...
static int free_base_req(uint64_t *num_inflight_reqs, nccl_ofi_freelist_t *nccl_ofi_reqs_fl, nccl_net_ofi_rdma_req_t *req, bool dec_inflight_reqs) {
  ...
  /* Update free list */
  if (nccl_ofi_reqs_fl == nullptr) [[unlikely]] {
    ret = -EINVAL;
  ...
  if ((dec_inflight_reqs == true) && (num_inflight_reqs != nullptr)) [[likely]]
    (*num_inflight_reqs)--;
...
static int free_write_req(nccl_net_ofi_rdma_req_t *req, bool dec_inflight_reqs) {
  ...
  auto *s_comm = (nccl_net_ofi_rdma_send_comm_t *)req->comm;
  return free_base_req(&s_comm->num_inflight_reqs, s_comm->nccl_ofi_reqs_fl, req, dec_inflight_reqs);
}
...
static int free_send_req(nccl_net_ofi_rdma_req_t *req, bool dec_inflight_reqs) {
  assert(req->type == NCCL_OFI_RDMA_SEND);
  auto *s_comm = (nccl_net_ofi_rdma_send_comm_t *)req->comm;
  rdma_req_send_data_t *send_data = nullptr;
  ...
  if (send_data->schedule) {
    const nccl_net_ofi_rdma_device_t *device = rdma_req_get_device(req);
    nccl_net_ofi_release_schedule(device->scheduler, send_data->schedule);
    send_data->schedule = nullptr;
  }
...
static int free_rx_buff_req(nccl_net_ofi_rdma_req_t *req, bool dec_inflight_reqs) {
  assert(!dec_inflight_reqs);
  const rdma_req_rx_buff_data_t *rx_buff_data = get_rx_buff_data(req);
  const nccl_net_ofi_rdma_ep_t *ep = rx_buff_data->ep;
  /* Free buffer */
  if (rx_buff_data->rx_buff_fl_elem) {
    nccl_ofi_freelist_entry_free(ep->rx_buff_fl, rx_buff_data->rx_buff_fl_elem);
  }
  return free_base_req(nullptr, ep->rx_buff_reqs_fl, req, false);
}

static nccl_net_ofi_rdma_req_t *alloc_rx_buff_req(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_ep_rail_t *rail) {
  ...
  nccl_ofi_freelist_elem_t *rx_buff_fl_elem = nccl_ofi_freelist_entry_alloc(ep->rx_buff_fl);
  if (!rx_buff_fl_elem) {
...
static int handle_rx_eagain(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_ep_rail_t *rail, nccl_net_ofi_rdma_req_t *req, size_t num_buffs_failed) {
  /* Add to pending reqs queue */
  const int ret = nccl_ofi_deque_insert_back(ep->pending_reqs_queue, &req->pending_reqs_elem);
  if (ret != 0) {
    NCCL_OFI_WARN("Failed to nccl_ofi_deque_insert_back: %d", ret);
...
static int post_rx_buffs_on_rail(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_ep_rail_t *rail) {
  ...
  const size_t buffers_needed = rail->max_rx_buff_posted - rail->num_rx_buff_posted;
  ...
  for (size_t i = 0; i < buffers_needed; ++i) {
    const bool is_last_req = (i == (buffers_needed - 1));
    nccl_net_ofi_rdma_req_t *req = alloc_rx_buff_req(ep, rail);
    if (!req) {
    ...
    if (ret == -FI_EAGAIN) {
      ...
      const size_t num_buffs_failed = buffers_needed - i - 1;
 ...
 */
static int post_rx_buffs(nccl_net_ofi_rdma_ep_t *ep) {
  int ret = 0;
  nccl_net_ofi_ep_rail_t *rail = nullptr;
 ...
 * 		error, others
 */
static int init_send_comm_rails(nccl_net_ofi_rdma_send_comm_t *s_comm, nccl_net_ofi_rdma_ep_t *ep, int dev_id, nccl_ofi_rdma_ep_name_t *remote_ep_names,
                                int num_remote_rails, nccl_ofi_rdma_ep_name_t *remote_control_ep_names, int num_remote_control_rails) {
  int ret = 0;
  nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t *comm_rail = nullptr;
  const nccl_net_ofi_ep_rail_t *ep_rail = nullptr;
  const nccl_ofi_rdma_ep_name_t *remote_rdma_ep_name = nullptr;
  ...
  for (int rail_id = s_comm->num_init_control_rails; rail_id < s_comm->num_control_rails; ++rail_id) {
    ...
    ret = fi_av_insert(ep_rail->av, (void *)remote_rdma_ep_name->ep_name, 1, &comm_rail->remote_addr, 0, nullptr);
    if (ret != 1) [[unlikely]] {
...
static int finish_connect(nccl_net_ofi_rdma_send_comm_t *s_comm) {
  ...
  const nccl_net_ofi_rdma_device_t *device = nullptr;

  assert(s_comm->conn_resp_req);
  if (s_comm->conn_resp_req->state != NCCL_OFI_RDMA_REQ_COMPLETED) {
    NCCL_OFI_WARN("Invalid connect response request state. Got %i but expected %i", s_comm->conn_resp_req->state, NCCL_OFI_RDMA_REQ_COMPLETED);
    return -EINVAL;
  }

  /* Validate endpoint */
  ep = (nccl_net_ofi_rdma_ep_t *)s_comm->base.base.ep;
  if (ep == nullptr) [[unlikely]] {
  ...
  if (conn_resp->num_rails != ep->num_rails) {
    NCCL_OFI_WARN("Unexpected number of remote rails for dev %d. Expected %i but got %i", dev_id, ep->num_rails, conn_resp->num_rails);
    return -EINVAL;
  }

  if (conn_resp->num_control_rails != ep->num_control_rails) {
    NCCL_OFI_WARN("Unexpected number of remote control rails for dev %d. Expected %i but got %i", dev_id, ep->num_control_rails, conn_resp->num_control_rails);
    return -EINVAL;
  }

  /* Validate received comm ID */
  if (conn_resp->local_comm_id >= device->num_comm_ids) [[unlikely]] {
    NCCL_OFI_WARN("Received an invalid communicator ID %u for device %d", conn_resp->local_comm_id, dev_id);
    return -EINVAL;
  }
  ...
  ret = init_send_comm_rails(s_comm, ep, dev_id, conn_resp->ep_names, conn_resp->num_rails, conn_resp->control_ep_names, conn_resp->num_control_rails);
  if (ret != 0) {
    return ret;
  }

  s_comm->conn_resp_req->free(s_comm->conn_resp_req, false);
  s_comm->conn_resp_req = nullptr;
...
static int test(nccl_net_ofi_req_t *base_req, int *done, int *size) {
  int ret = 0;
  auto *req = (nccl_net_ofi_rdma_req_t *)base_req;
  *done = 0;
  assert(req->type == NCCL_OFI_RDMA_WRITE || req->type == NCCL_OFI_RDMA_READ || req->type == NCCL_OFI_RDMA_SEND || req->type == NCCL_OFI_RDMA_RECV ||
  ...
  auto *ep = (nccl_net_ofi_rdma_ep_t *)base_comm->ep;
  assert(ep != NULL);

  /* Process more completions unless the current request is
   * completed */
  if (req->state != NCCL_OFI_RDMA_REQ_COMPLETED && req->state != NCCL_OFI_RDMA_REQ_ERROR) [[likely]] {
  ...
  if (req->state == NCCL_OFI_RDMA_REQ_COMPLETED) [[likely]] {
    ...
    *done = 1;
    ...
    if (req->type == NCCL_OFI_RDMA_SEND || req->type == NCCL_OFI_RDMA_RECV) {
      ...
      nccl_ofi_msgbuff_status_t stat;
      const nccl_ofi_msgbuff_result_t mb_res = nccl_ofi_msgbuff_complete(msgbuff, req->msg_seq_num, &stat);
      if (mb_res != NCCL_OFI_MSGBUFF_SUCCESS) [[unlikely]] {
    ...
    assert(req->free);
    req->free(req, true);
  } else if (req->state == NCCL_OFI_RDMA_REQ_ERROR) [[unlikely]] {
    ret = -EINVAL;
    goto exit;
  }
 ...
 *		by recv connect response request
 *
 * @param	l_comm
...
static int prepare_recv_conn_req(nccl_net_ofi_rdma_listen_comm_t *l_comm) {
  ...
  ret = nccl_net_ofi_mutex_init(&req->req_lock, nullptr);
  if (ret != 0) [[unlikely]] {
...
static int dereg_rails(nccl_net_ofi_rdma_mr_handle_t *handle) {
  int ret = 0;
  int rc = 0;
  const int num_rails = handle->num_rails;
  const int num_control_rails = handle->num_control_rails;
  ...
  for (int rail_id = 0; rail_id != num_control_rails; ++rail_id) {
    ...
    if (!handle->control_mr[rail_id])
      continue;
    rc = fi_close(&handle->control_mr[rail_id]->fid);
    if (rc != 0) [[unlikely]] {
      NCCL_OFI_WARN("Unable to de-register memory on control MR. RC: %d, Error: %s", rc, fi_strerror(-rc));
      ret = rc;
    }
  }
  ...
  for (int rail_id = 0; rail_id != num_rails; ++rail_id) {
    ...
    if (rc != 0) [[unlikely]] {
      NCCL_OFI_WARN("Unable to de-register memory. RC: %d, Error: %s", rc, fi_strerror(-rc));
      ret = rc;
    }
  }
...
static void free_rdma_mr_handle(nccl_net_ofi_rdma_mr_handle_t *handle) {
  if (handle) {
 ...
 *		NULL, on error
 */
static nccl_net_ofi_rdma_mr_handle_t *calloc_rdma_mr_handle(int num_rails, int num_control_rails) {
  auto *ret_handle = (nccl_net_ofi_rdma_mr_handle_t *)calloc(1, sizeof(nccl_net_ofi_rdma_mr_handle_t));

  if (!ret_handle) [[unlikely]] {
    NCCL_OFI_WARN("Unable to allocate memory registration handle");
    goto error;
  }

  ret_handle->mr = (fid_mr **)calloc(num_rails, sizeof(fid_mr *));
  if (!ret_handle->mr) [[unlikely]] {
    NCCL_OFI_WARN("Unable to allocate memory registration handles array");
    goto error;
  }

  ret_handle->control_mr = (fid_mr **)calloc(num_control_rails, sizeof(fid_mr *));
  if (!ret_handle->control_mr) [[unlikely]] {
 ...
 *		non-zero on error
 */
static int dereg_mr_ep(nccl_net_ofi_rdma_mr_handle_t *mr_handle, nccl_ofi_idpool_t *key_pool, nccl_ofi_mr_cache_t *mr_cache) {
  int ret = 0;

  if (mr_handle == nullptr) [[unlikely]] {
    NCCL_OFI_WARN("Null MR handle provided. This is an error.");
    return -EINVAL;
  }

  if (mr_handle->num_rails < 0) [[unlikely]] {
  ...
  if (nccl_ofi_idpool_active(key_pool)) {
    const uint64_t key = fi_mr_key(mr_handle->mr[0]);
...
static int reg_mr_on_device(nccl_net_ofi_rdma_ep_t *ep, nccl_ofi_mr_ckey_ref ckey, int type, nccl_net_ofi_rdma_mr_handle_t **mhandle) {
  int ret = 0;
  ...
  *mhandle = nullptr;
  ...
  const int dev_id = device->base.dev_id;
  const int num_rails = ep->num_rails;
  const int num_control_rails = ep->num_control_rails;
  ...
  for (int rail_id = 0; rail_id != num_rails; ++rail_id) {
    const nccl_net_ofi_ep_rail_t *rail = rdma_endpoint_get_rail(ep, rail_id);
    ofi_domain = rdma_endpoint_get_ofi_domain(ep, rail_id);

    ret = register_rail_mr_buffer(ofi_domain, rail->ofi_ep, dev_id, type, &mr_attr, regattr_flags, &ret_handle->mr[rail_id]);
    if (ret != 0) [[unlikely]] {
      if (dereg_mr_ep(ret_handle, key_pool, nullptr) != 0) {
  ...
  for (int rail_id = 0; rail_id != num_control_rails; ++rail_id) {
    const nccl_net_ofi_ep_rail_t *rail = rdma_endpoint_get_control_rail(ep, rail_id);
    ofi_domain = rdma_endpoint_get_ofi_control_domain(ep, rail_id);

    ret = register_rail_mr_buffer(ofi_domain, rail->ofi_ep, dev_id, type, &mr_attr, regattr_flags, &ret_handle->control_mr[rail_id]);
    if (ret != 0) [[unlikely]] {
      if (dereg_mr_ep(ret_handle, key_pool, nullptr) != 0) {
        NCCL_OFI_WARN("Error de-registering MR");
      }
 ...
 */
static int reg_mr_ep(nccl_net_ofi_rdma_ep_t *ep, nccl_ofi_mr_ckey_ref ckey, int type, nccl_ofi_mr_cache_t *mr_cache, nccl_net_ofi_rdma_mr_handle_t **mhandle) {
  int ret = 0;
  nccl_net_ofi_rdma_mr_handle_t *ret_handle = nullptr;
  *mhandle = nullptr;
...
static int reg_internal_mr_ep(nccl_net_ofi_rdma_ep_t *ep, void *data, size_t size, int type, nccl_net_ofi_rdma_mr_handle_t **mhandle) {
  ...
  return reg_mr_ep(ep, &ckey, type, nullptr, mhandle);
}

static int reg_mr_send_comm(nccl_net_ofi_send_comm_t *send_comm, nccl_ofi_mr_ckey_ref ckey, int type, void **mhandle) {
  auto *ep = (nccl_net_ofi_rdma_ep_t *)send_comm->base.ep;
  const nccl_net_ofi_rdma_domain_t *domain = rdma_endpoint_get_domain(ep);
  assert(domain != NULL);

  return reg_mr_ep(ep, ckey, type, domain->base.mr_cache, (nccl_net_ofi_rdma_mr_handle_t **)mhandle);
}

static int reg_mr_recv_comm(nccl_net_ofi_recv_comm_t *recv_comm, nccl_ofi_mr_ckey_ref ckey, int type, void **mhandle) {
  auto *ep = (nccl_net_ofi_rdma_ep_t *)recv_comm->base.ep;
  const nccl_net_ofi_rdma_domain_t *domain = rdma_endpoint_get_domain(ep);
  assert(domain != NULL);
...
using freelist_regmr_fn_handle_t = struct {
...
static int freelist_regmr_host_fn(void *ep_void_ptr, void *data, size_t size, void **handle) {
  auto *ep = (nccl_net_ofi_rdma_ep_t *)ep_void_ptr;

  nccl_net_ofi_rdma_mr_handle_t *mr_handle = nullptr;
  const int ret = reg_internal_mr_ep(ep, data, size, NCCL_PTR_HOST, &mr_handle);
  ...
  auto *freelist_handle = (freelist_regmr_fn_handle_t *)malloc(sizeof(freelist_regmr_fn_handle_t));
  if (!freelist_handle) {
...
static int freelist_deregmr_host_fn(void *handle) {
  auto *freelist_handle = (freelist_regmr_fn_handle_t *)handle;
  assert(freelist_handle);
  const int ret = dereg_mr_ep(freelist_handle->mr_handle, freelist_handle->key_pool, nullptr);
  if (ret != 0) [[unlikely]] {
...
static int dereg_mr_recv_comm(nccl_net_ofi_recv_comm_t *recv_comm, nccl_net_ofi_mr_handle_t *mhandle) {
  /* Retrieve and validate endpoint */
  auto *ep = (nccl_net_ofi_rdma_ep_t *)recv_comm->base.ep;
  assert(ep != NULL);
  ...
  auto *mr_handle = (nccl_net_ofi_rdma_mr_handle_t *)mhandle;
...
static nccl_net_ofi_rdma_req_t *allocate_req(nccl_ofi_freelist_t *fl) {
  ...
  auto *req = (nccl_net_ofi_rdma_req_t *)elem->ptr;
  assert(req);
  ...
  /* Initialize mutex for request access */
  if (nccl_net_ofi_mutex_init(&req->req_lock, nullptr)) {
    NCCL_OFI_WARN("Unable to initialize mutex");
    goto cleanup;
  }
 ...
 */
static int insert_send_ctrl_req(nccl_net_ofi_rdma_recv_comm_t *r_comm, nccl_net_ofi_rdma_device_t *device, int dev_id, uint16_t msg_seq_num, void *buff,
                                size_t size, nccl_net_ofi_rdma_mr_handle_t *buff_mr_handle, nccl_net_ofi_rdma_req_t *recv_req) {
  nccl_net_ofi_scheduler_t *scheduler = device->scheduler;
  const auto *ep = (nccl_net_ofi_rdma_ep_t *)r_comm->base.base.ep;
  nccl_net_ofi_rdma_req_t *send_ctrl_req = allocate_req(r_comm->nccl_ofi_reqs_fl);
  if (send_ctrl_req == nullptr) [[unlikely]] {
  ...
  if (ep->num_control_rails > 1) {
    const size_t ctrl_msg_len = nccl_net_ofi_rdma_ctrl_msg_size(ep->num_rails, ep->use_long_rkeys);
    send_ctrl_data->ctrl_schedule = scheduler->get_schedule(scheduler, ctrl_msg_len, ep->num_control_rails);

    if (!(send_ctrl_data->ctrl_schedule)) [[unlikely]] {
      return -EINVAL;
    } else if (send_ctrl_data->ctrl_schedule->num_xfer_infos != 1) [[unlikely]] {
      NCCL_OFI_WARN("Invalid schedule for outgoing control message (%zu bytes). Expected one rail, but got "
                    "%zu",
                    size, send_ctrl_data->ctrl_schedule->num_xfer_infos);
      return -EINVAL;
    }
   ...
   */
  send_ctrl_data->ctrl_fl_elem = nccl_ofi_freelist_entry_alloc(r_comm->ctrl_buff_fl);
  if (send_ctrl_data->ctrl_fl_elem == nullptr) {
  ...
  for (; rail_id < r_comm->num_rails; rail_id++) {
    const uint64_t rkey = fi_mr_key(buff_mr_handle->mr[rail_id]);
    ...
    if (ep->use_long_rkeys) {
      ...
      if (rkey > (1ULL << (NCCL_NET_OFI_CTRL_MSG_SHORT_KEY_SIZE * 8)) - 1) {
        NCCL_OFI_WARN("Libfabric returned rkey larger than declared rkey size: %" PRIu64, rkey);
        return -ENOTSUP;
      }
 ...
 */
static int insert_recv_segms_req(nccl_net_ofi_rdma_recv_comm_t *r_comm, nccl_net_ofi_rdma_device_t *device, int dev_id, uint16_t msg_seq_num, void *buff,
                                 size_t size, nccl_net_ofi_rdma_req_t *recv_req) {
  /* Allocate recv segms request */
  nccl_net_ofi_rdma_req_t *recv_segms_req = allocate_req(r_comm->nccl_ofi_reqs_fl);
  if (recv_segms_req == nullptr) [[unlikely]] {
    NCCL_OFI_WARN("Unable to get NCCL OFI receive segments request for device %d", dev_id);
    return -ENOENT;
  }
 ...
 * @brief	Allocate a new recv req from freelist
 */
static int allocate_rdma_recv_req(nccl_net_ofi_rdma_recv_comm_t *r_comm, nccl_net_ofi_rdma_device_t *device, int dev_id, uint16_t msg_seq_num, void *buff,
                                  size_t size, nccl_net_ofi_rdma_mr_handle_t *buff_mr_handle, nccl_net_ofi_rdma_req_t **ret_req) {
  int ret = 0;
...
static int insert_rdma_recv_req_into_msgbuff(nccl_net_ofi_rdma_recv_comm_t *r_comm, bool eager, nccl_net_ofi_rdma_req_t **ret_req) {
  ...
  if (eager) {
     ...
     */
    mb_res = nccl_ofi_msgbuff_replace(r_comm->msgbuff, req->msg_seq_num, req, NCCL_OFI_MSGBUFF_REQ, &msg_stat);
    if (mb_res != NCCL_OFI_MSGBUFF_SUCCESS) [[unlikely]] {
      NCCL_OFI_WARN("Unexpected result of nccl_ofi_msgbuff_replace for msg %hu", req->msg_seq_num);
      return -EINVAL;
    }

  } else {
    /* Try inserting the new request */
    mb_res = nccl_ofi_msgbuff_insert(r_comm->msgbuff, req->msg_seq_num, req, NCCL_OFI_MSGBUFF_REQ, &msg_stat);

    if ((mb_res == NCCL_OFI_MSGBUFF_INVALID_IDX) && (msg_stat == NCCL_OFI_MSGBUFF_INPROGRESS)) [[unlikely]] {
      /* Unlikely: an eager message was received on another
         thread. Return NULL and let NCCL call recv again. */
      req->free(req, false);
      *ret_req = nullptr;
    } else if (mb_res != NCCL_OFI_MSGBUFF_SUCCESS) [[unlikely]] {
      NCCL_OFI_WARN("Unexpected result of nccl_ofi_msgbuff_insert for msg %hu", req->msg_seq_num);
      return -EINVAL;
    }
  }
  return 0;
...
static int process_cq_if_pending(nccl_net_ofi_rdma_ep_t *ep) {
  ...
  if (!nccl_ofi_deque_isempty(ep->pending_reqs_queue)) {
    const int ret = ofi_process_cq(ep);
    if (ret != 0) {
      return ret;
    }
...
static int recv(nccl_net_ofi_recv_comm_t *recv_comm, int n, void **buffers, int *sizes, int *tags, nccl_net_ofi_mr_handle_t **mhandles,
  ...
  auto *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)recv_comm;
  rdma_req_recv_data_t *recv_data = nullptr;
  nccl_net_ofi_rdma_ep_t *ep = nullptr;
  nccl_net_ofi_rdma_device_t *device = nullptr;
  int dev_id = 0;
  auto **mr_handles = (nccl_net_ofi_rdma_mr_handle_t **)mhandles;
  ...
  if (mb_res == NCCL_OFI_MSGBUFF_SUCCESS) {
    ...
    if (type == NCCL_OFI_MSGBUFF_REQ) {
      ...
      ret = -EINVAL;
      return onError();
    } else if (type == NCCL_OFI_MSGBUFF_BUFF) [[likely]] {
  ...
  } else if ((mb_res == NCCL_OFI_MSGBUFF_INVALID_IDX) && (msg_stat == NCCL_OFI_MSGBUFF_NOTSTARTED)) {
    /* Allocate a new req */
  } else {
  ...
  ret = allocate_rdma_recv_req(r_comm, device, dev_id, msg_seq_num, buffers[0], sizes[0], mr_handles[0], &req);
  if (ret != 0) {
    return onError();
  }
  ...
  if (eager) {
    auto *rx_buff_req = (nccl_net_ofi_rdma_req_t *)elem;
    const rdma_req_rx_buff_data_t *rx_buff_data = get_rx_buff_data(rx_buff_req);
    if (rx_buff_data->recv_len == 0) {
...
static bool is_flush_buff_enabled() { return !ofi_nccl_gdr_flush_disable() && support_gdr == GDR_SUPPORTED && !cuda_flush; }
 ...
 * 		error, on others
 */
static int dealloc_and_dereg_flush_buff(nccl_net_ofi_rdma_recv_comm_t *r_comm, nccl_net_ofi_rdma_domain_t *domain) {
  int ret = 0;
  nccl_net_ofi_rdma_mr_handle_t *mr_handle = r_comm->flush_buff.mr_handle;

  if (mr_handle) {
    ret = dereg_mr_ep(mr_handle, &domain->base.mr_rkey_pool, nullptr);
  }
  if (ret != 0) {
...
static int alloc_and_reg_flush_buff(nccl_net_ofi_rdma_recv_comm_t *r_comm, int dev_id) {
  ...
  int rc = 0;
  nccl_net_ofi_rdma_mr_handle_t *mr_handle = nullptr;
  nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_flush_buffer_t *flush_buff = &r_comm->flush_buff;
  auto *ep = (nccl_net_ofi_rdma_ep_t *)r_comm->base.base.ep;
  ...
  if (local_mr == true) {
    ...
    ret = reg_internal_mr_ep(ep, flush_buff->host_buffer, system_page_size, NCCL_PTR_HOST, &mr_handle);
    if (ret != 0) [[unlikely]] {
      NCCL_OFI_WARN("Could not register dummy buffer for flush, dev: %d", dev_id);
      rc = nccl_net_ofi_dealloc_mr_buffer(flush_buff->host_buffer, system_page_size);
      if (rc != 0) {
    ...
    NCCL_OFI_TRACE(NCCL_NET, "Skip registering host buffer. local_mr: %d", local_mr);
  }
...
static void free_rdma_recv_comm(nccl_net_ofi_rdma_recv_comm_t *r_comm) {
  if (r_comm) {
...
static int recv_comm_destroy(nccl_net_ofi_rdma_recv_comm_t *r_comm) {
  ...
  if (r_comm->send_close_req != nullptr) {
  ...
  rdma_device_set_comm(device, r_comm->local_comm_id, nullptr);
 ...
 * member
 */
static int recv_comm_insert_send_close_req(nccl_net_ofi_rdma_recv_comm_t *r_comm) {
  auto *ep = (nccl_net_ofi_rdma_ep_t *)r_comm->base.base.ep;
  const nccl_net_ofi_rdma_device_t *device = rdma_endpoint_get_device(ep);
  nccl_net_ofi_scheduler_t *scheduler = device->scheduler;
  nccl_net_ofi_rdma_req_t *send_close_req = allocate_req(r_comm->nccl_ofi_reqs_fl);
  if (send_close_req == nullptr) [[unlikely]] {
    return -ENOMEM;
  }
  ...
  send_close_data->ctrl_schedule = scheduler->get_schedule(scheduler, sizeof(nccl_net_ofi_rdma_close_msg_t), device->num_rails);
  if (!(send_close_data->ctrl_schedule)) [[unlikely]] {
    send_close_req->free(send_close_req, false);
    return -ENOMEM;
  } else if (send_close_data->ctrl_schedule->num_xfer_infos != 1) [[unlikely]] {
    NCCL_OFI_WARN("Invalid schedule for outgoing close message (%zu bytes). Expected one rail, but got %zu", sizeof(nccl_net_ofi_rdma_close_msg_t),
   ...
   */
  send_close_data->ctrl_fl_elem = nccl_ofi_freelist_entry_alloc(r_comm->ctrl_buff_fl);
  if (send_close_data->ctrl_fl_elem == nullptr) {
    NCCL_OFI_WARN("Call to nccl_ofi_freelist_entry_alloc failed");
    send_close_req->free(send_close_req, false);
    return -ENOMEM;
  }
 ...
 * this function
 */
static int recv_comm_process_all_finalizing() {
  int ret = 0;

  NCCL_OFI_DEQUE_FOREACH(r_comm_cleanup_list) {

    nccl_net_ofi_rdma_recv_comm_t *r_comm = container_of(elem, nccl_net_ofi_rdma_recv_comm_t, cleanup_list_elem);
    ...
    if (r_comm->send_close_req == nullptr) {
      ...
      nccl_net_ofi_mutex_lock(&r_comm->ctrl_counter_lock);
      const bool all_ctrl_msgs_delivered = (r_comm->n_ctrl_delivered == r_comm->n_ctrl_sent);
      nccl_net_ofi_mutex_unlock(&r_comm->ctrl_counter_lock);
      ...
      const nccl_net_ofi_rdma_req_state_t state = r_comm->send_close_req->state;
...
static void free_rdma_send_comm(nccl_net_ofi_rdma_send_comm_t *s_comm) {
  if (s_comm) {
...
static int send_comm_destroy(nccl_net_ofi_rdma_send_comm_t *s_comm) {
  ...
  auto *ep = (nccl_net_ofi_rdma_ep_t *)s_comm->base.base.ep;
  nccl_net_ofi_rdma_device_t *device = rdma_endpoint_get_device(ep);
  rdma_device_set_comm(device, s_comm->local_comm_id, nullptr);
 ...
 * this function
 */
static int send_comm_process_all_finalizing() {
  int ret = 0;

  NCCL_OFI_DEQUE_FOREACH(s_comm_cleanup_list) {

    nccl_net_ofi_rdma_send_comm_t *s_comm = container_of(elem, nccl_net_ofi_rdma_send_comm_t, cleanup_list_elem);
    ...
    const bool ready_to_destroy = (s_comm->received_close_message) && (s_comm->n_ctrl_received == s_comm->n_ctrl_expected);
 ...
 * this function
 */
static int comm_close_handler() {
  int ret = 0;

  while (!(nccl_ofi_deque_isempty(s_comm_cleanup_list)) || !(nccl_ofi_deque_isempty(r_comm_cleanup_list))) {
...
static int recv_close_deferred(nccl_net_ofi_recv_comm_t *recv_comm) {
  ...
  nccl_ofi_deque_insert_back(r_comm_cleanup_list, &r_comm->cleanup_list_elem);
...
static int rdma_comm_alloc_flush_req(nccl_net_ofi_rdma_recv_comm_t *r_comm, void *buff, nccl_net_ofi_rdma_mr_handle_t *buff_mr_handle,
                                     nccl_net_ofi_rdma_req_t **ret_req) {
  const auto *ep = (nccl_net_ofi_rdma_ep_t *)r_comm->base.base.ep;
  const int dev_id = r_comm->base.base.dev_id;
  rdma_req_flush_data_t *flush_data = nullptr;
  *ret_req = nullptr;
...
static int flush(nccl_net_ofi_recv_comm_t *recv_comm, int n, void **buffers, int *sizes, nccl_net_ofi_mr_handle_t **mhandles, nccl_net_ofi_req_t **base_req) {
  int ret = 0;
  int flush_n = 0;
 ...
 *		NULL, on error
 */
static nccl_net_ofi_rdma_recv_comm_t *calloc_rdma_recv_comm(int num_rails, int num_control_rails) {
  auto *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)calloc(1, sizeof(nccl_net_ofi_rdma_recv_comm_t));
  if (!r_comm) [[unlikely]] {
    NCCL_OFI_WARN("Unable to allocate receive communicator");
    goto error;
  }

  r_comm->rails = (nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_recv_comm_rail_t *)calloc(
      num_rails, sizeof(nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_recv_comm_rail_t));
  if (!r_comm->rails) [[unlikely]] {
    NCCL_OFI_WARN("Unable to allocate receive communicator rails array");
    goto error;
  }

  r_comm->control_rails = (nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_recv_comm_rail_t *)calloc(
      num_control_rails, sizeof(nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_recv_comm_rail_t));
  if (!r_comm->control_rails) [[unlikely]] {
...
static void init_rma_op_req(nccl_net_ofi_rdma_req_t *req, nccl_net_ofi_comm_t *comm, void *buff, size_t size, void *desc, uint64_t remote_buff,
                            uint64_t remote_mr_key, uint64_t flags, nccl_net_ofi_rdma_req_type_t req_type) {
  req->comm = comm;
...
static int alloc_rdma_read_req(nccl_net_ofi_rdma_recv_comm_t *r_comm, nccl_net_ofi_rdma_ep_t *ep, void *buff, size_t size,
                               nccl_net_ofi_rdma_mr_handle_t *buff_mr_handle, uint64_t remote_buff, uint64_t remote_mr_key, nccl_net_ofi_rdma_req_t **ret_req) {
  const uint64_t flags = 0;
  fid_mr *rail_mr_handle = buff_mr_handle->mr[0];
  ...
  *ret_req = nullptr;
  ...
  init_rma_op_req(req, &r_comm->base.base, buff, size, desc, remote_buff, remote_mr_key, flags, NCCL_OFI_RDMA_READ);
...
static int rma_read(nccl_net_ofi_recv_comm_t *recv_comm, void *dest, size_t size, void *mhandle, uint64_t src, uint64_t mr_key, nccl_net_ofi_req_t **base_req) {
  int ret = 0;
  auto *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)recv_comm;
  auto *mr_handle = (nccl_net_ofi_rdma_mr_handle_t *)mhandle;
  nccl_net_ofi_rdma_req_t *req = nullptr;
  ...
  ret = alloc_rdma_read_req(r_comm, ep, dest, size, mr_handle, src, mr_key, &req);
  if (ret != 0) [[unlikely]] {
    goto error;
  }
 ...
 * 		NULL, on error
 */
static nccl_net_ofi_rdma_recv_comm_t *prepare_recv_comm(nccl_net_ofi_rdma_domain_t *domain, nccl_net_ofi_rdma_ep_t *l_comm_ep,
  ...
  const int dev_id = device->base.dev_id;
  const int num_rails = l_comm_ep->num_rails;
  const int num_control_rails = l_comm_ep->num_control_rails;
  ...
  ret = nccl_net_ofi_mutex_init(&r_comm->ctrl_counter_lock, nullptr);
  if (ret != 0) {
    free_rdma_recv_comm(r_comm);
    return nullptr;
  }
  ...
  /* Validate received comm ID */
  if (conn_msg->local_comm_id >= device->num_comm_ids) [[unlikely]] {
    NCCL_OFI_WARN("Received an invalid communicator ID %" PRIu32 " for device %d", conn_msg->local_comm_id, dev_id);
    goto error;
  }
  ...
  if (ofi_nccl_endpoint_per_communicator() != 0) {
    ...
    nccl_net_ofi_ep_t *ep_for_addr = nullptr;
    ret = nccl_ofi_ep_addr_list_get(domain->ep_addr_list, remote_rail0_ep_name->ep_name, remote_rail0_ep_name->ep_name_len, &ep_for_addr);
    if (ret != 0) {
      goto error;
    }

    if (ep_for_addr == nullptr) {
      nccl_net_ofi_ep_t *new_base_ep = nullptr;
      ...
      auto *new_ep = (nccl_net_ofi_rdma_ep_t *)new_base_ep;
      new_ep->is_endpoint_per_communicator_ep = true;

      ep_for_addr = &new_ep->base;

      ret = nccl_ofi_ep_addr_list_insert(domain->ep_addr_list, ep_for_addr, remote_rail0_ep_name->ep_name, remote_rail0_ep_name->ep_name_len);
      if (ret != 0) {
        goto error;
      }
    }
  ...
  for (int rail_id = 0; rail_id != num_control_rails; ++rail_id) {
    nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_recv_comm_rail_t *comm_rail = rdma_recv_comm_get_control_rail(r_comm, rail_id);
    const nccl_net_ofi_ep_rail_t *rail = rdma_endpoint_get_control_rail(ep, rail_id);
    ...
    /* Insert remote EP address to AV */
    ret = fi_av_insert(rail->av, (void *)remote_ep_name->ep_name, 1, &comm_rail->remote_addr, 0, nullptr);
    if (ret != 1) [[unlikely]] {
      NCCL_OFI_WARN("Unable to insert remote address into address vector "
                    "for device %d. RC: %s",
                    dev_id, fi_strerror(-ret));
    ...
    ret = fi_av_insert(rail->av, (void *)rail->local_ep_name, 1, &comm_rail->local_addr, 0, nullptr);
    if (ret != 1) [[unlikely]] {
  ...
  for (int rail_id = 0; rail_id != num_rails; ++rail_id) {
    nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_recv_comm_rail_t *comm_rail = rdma_recv_comm_get_rail(r_comm, rail_id);
    const nccl_net_ofi_ep_rail_t *rail = rdma_endpoint_get_rail(ep, rail_id);
  ...
  ret = nccl_ofi_freelist_init(sizeof(nccl_net_ofi_rdma_req_t), 16, 16, static_cast<size_t>(4 * NCCL_OFI_MAX_REQUESTS), &r_comm->nccl_ofi_reqs_fl);
  if (ret != 0) [[unlikely]] {
 ...
 *		-EiNVAL, on others
 */
static int prepare_conn_resp(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_rdma_listen_comm_t *l_comm, int dev_id) {
  const int num_rails = ep->num_rails;
  const int num_control_rails = ep->num_control_rails;
  nccl_ofi_rdma_connection_info_t *conn_resp = &l_comm->conn_msg;
  nccl_ofi_rdma_ep_name_t *rdma_ep_name = nullptr;
  const nccl_net_ofi_ep_rail_t *ep_rail = nullptr;
  ...
  for (int rail_id = 0; rail_id != num_rails; ++rail_id) {
    ...
    memcpy(rdma_ep_name->ep_name, ep_rail->local_ep_name, ep_rail->local_ep_name_len);
 ...
 * 		others, on error
 */
static int post_send_conn_resp(nccl_net_ofi_rdma_recv_comm_t *r_comm, nccl_ofi_rdma_connection_info_t *conn_resp, nccl_net_ofi_rdma_device_t *device,
                               nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_rdma_req_t *req) {
  ssize_t rc = 0;
  const nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_recv_comm_rail_t *comm_rail = rdma_recv_comm_get_control_rail(r_comm, 0);

  req->state = NCCL_OFI_RDMA_REQ_PENDING;
  rc = fi_send(comm_rail->local_ep, (void *)conn_resp, sizeof(nccl_ofi_rdma_connection_info_t), nullptr, comm_rail->remote_addr, req);
  ...
  if (rc == -FI_EAGAIN) {
     ...
     */
    const int res = ofi_process_cq(ep);
    if (res != 0)
      return res;
  } else if (rc != 0) {
    req->state = NCCL_OFI_RDMA_REQ_CREATED;
    NCCL_OFI_WARN("Unable to send connect message for dev %d. RC: %zd, ERROR: %s", device->base.dev_id, rc, fi_strerror(-rc));
  }
...
static int accept(nccl_net_ofi_listen_comm_t *listen_comm, nccl_net_ofi_recv_comm_t **recv_comm) {
  ...
  auto *l_comm = (nccl_net_ofi_rdma_listen_comm_t *)listen_comm;
  ...
  /* Retrieve and validate endpoint */
  auto *l_comm_ep = (nccl_net_ofi_rdma_ep_t *)l_comm->base.base.ep;
  ...
  if (l_comm->stage == nccl_ofi_comm_stage_t::COMM_CONNECTED) {
    NCCL_OFI_WARN("listenComm %p object already has an active connection (%d).", l_comm, l_comm->stage);
    ret = -EINVAL;
    goto exit;
  }
  ...
  *recv_comm = nullptr;
  ...
  case nccl_ofi_comm_stage_t::COMM_CREATE_START:
    /* nccl_ofi_comm_stage_t::COMM_CREATE_START:Allocate data required for the accept function */

    l_comm->stage = nccl_ofi_comm_stage_t::COMM_RECV_CONN;

    [[fallthrough]];
  case nccl_ofi_comm_stage_t::COMM_RECV_CONN:

    l_comm->stage = nccl_ofi_comm_stage_t::COMM_CONN_REQ_PENDING;

    [[fallthrough]];
  case nccl_ofi_comm_stage_t::COMM_CONN_REQ_PENDING:
    /* nccl_ofi_comm_stage_t::COMM_CONN_REQ_PENDING: Wait until connect message has been
    ...
    if (conn_msg->num_rails != l_comm_ep->num_rails) {
      NCCL_OFI_WARN("Unexpected number of remote rails for dev %d. Expected %i but got %i", dev_id, l_comm_ep->num_rails, conn_msg->num_rails);
      ret = -EINVAL;
      goto exit;
    }
    ...
    if (conn_msg->num_control_rails != l_comm_ep->num_control_rails) {
      NCCL_OFI_WARN("Unexpected number of remote control rails for dev %d. Expected %i but got %i", dev_id, l_comm_ep->num_control_rails,
    ...
    l_comm->stage = nccl_ofi_comm_stage_t::COMM_SEND_CONN;

    [[fallthrough]];
  case nccl_ofi_comm_stage_t::COMM_SEND_CONN:

    /* nccl_ofi_comm_stage_t::COMM_SEND_CONN: Send connect response message to remote */
    ...
    l_comm->stage = nccl_ofi_comm_stage_t::COMM_CONN_RESP_REQ_PENDING;

    [[fallthrough]];
  case nccl_ofi_comm_stage_t::COMM_CONN_RESP_REQ_PENDING:
    /* nccl_ofi_comm_stage_t::COMM_CONN_RESP_REQ_PENDING: Wait until connect
    ...
    *recv_comm = &r_comm->base;
    ...
    l_comm->stage = nccl_ofi_comm_stage_t::COMM_CONNECTED;
...
exit:;
  ...
  const int close_ret = close_listen_recv_comm(l_comm);
  if (close_ret) {
...
static int listen_close(nccl_net_ofi_listen_comm_t *listen_comm) {
  ...
  /* Release communicator ID */
  ret = nccl_ofi_idpool_free_id(rdma_endpoint_get_device((nccl_net_ofi_rdma_ep_t *)base_ep)->comm_idpool, l_comm->comm_id);
  if (ret != 0) [[unlikely]] {
...
static int listen(nccl_net_ofi_ep_t *base_ep, nccl_net_ofi_conn_handle_t *handle, nccl_net_ofi_listen_comm_t **listen_comm) {
  int ret = 0;
  nccl_net_ofi_rdma_listen_comm_t *l_comm = nullptr;
  int comm_id = 0;
  auto *ep = (nccl_net_ofi_rdma_ep_t *)base_ep;
  const nccl_net_ofi_ep_rail_t *first_control_rail = rdma_endpoint_get_control_rail(ep, 0);
  ...
  memcpy(handle->ep_name, first_control_rail->local_ep_name, first_control_rail->local_ep_name_len);
  ...
  memset(handle->ep_name + first_control_rail->local_ep_name_len, 0, sizeof(handle->ep_name) - first_control_rail->local_ep_name_len);

  /* Build listen_comm */
  l_comm = (nccl_net_ofi_rdma_listen_comm_t *)calloc(1, sizeof(nccl_net_ofi_rdma_listen_comm_t));
  if (l_comm == nullptr) [[unlikely]] {
...
static int dereg_mr_send_comm(nccl_net_ofi_send_comm_t *send_comm, nccl_net_ofi_mr_handle_t *mhandle) {
  /* Retrieve and validate endpoint */
  auto *ep = (nccl_net_ofi_rdma_ep_t *)send_comm->base.ep;
  assert(ep != NULL);
...
static int alloc_rdma_write_req(nccl_net_ofi_rdma_send_comm_t *s_comm, nccl_net_ofi_rdma_ep_t *ep, void *buff, size_t size, void *desc, uint64_t remote_buff,
                                uint64_t remote_mr_key, uint64_t flags, nccl_net_ofi_rdma_req_t **ret_req) {
  *ret_req = nullptr;
  ...
  init_rma_op_req(req, &s_comm->base.base, buff, size, desc, remote_buff, remote_mr_key, flags, NCCL_OFI_RDMA_WRITE);
...
static int alloc_rdma_send_req(nccl_net_ofi_rdma_send_comm_t *s_comm, uint16_t msg_seq_num, void *buff, size_t size,
                               nccl_net_ofi_rdma_mr_handle_t *buff_mr_handle, bool eager, nccl_net_ofi_rdma_req_t **ret_req) {
  auto *ep = (nccl_net_ofi_rdma_ep_t *)s_comm->base.base.ep;
  const nccl_net_ofi_rdma_device_t *device = rdma_endpoint_get_device(ep);
  nccl_net_ofi_scheduler_t *scheduler = device->scheduler;
  *ret_req = nullptr;
...
static int insert_rdma_send_req_into_msgbuff(nccl_net_ofi_rdma_send_comm_t *s_comm, int dev_id, bool have_ctrl, nccl_net_ofi_rdma_req_t **ret_req) {
  nccl_net_ofi_rdma_req_t *req = *ret_req;
  nccl_ofi_msgbuff_status_t msg_stat;
  nccl_ofi_msgbuff_result_t mb_res;
  ...
  if (have_ctrl) {
     ...
     */
    mb_res = nccl_ofi_msgbuff_replace(s_comm->msgbuff, req->msg_seq_num, req, NCCL_OFI_MSGBUFF_REQ, &msg_stat);
    if (mb_res != NCCL_OFI_MSGBUFF_SUCCESS) [[unlikely]] {
      NCCL_OFI_WARN("Unexpected result of nccl_ofi_msgbuff_replace for msg %hu", req->msg_seq_num);
      return -EINVAL;
    }
    ...
    /* Try inserting the new request */
    mb_res = nccl_ofi_msgbuff_insert(s_comm->msgbuff, req->msg_seq_num, req, NCCL_OFI_MSGBUFF_REQ, &msg_stat);
    if ((mb_res == NCCL_OFI_MSGBUFF_INVALID_IDX) && (msg_stat == NCCL_OFI_MSGBUFF_INPROGRESS)) [[unlikely]] {
...
static int post_rma_write(nccl_net_ofi_rdma_req_t *req) {
  auto *s_comm = (nccl_net_ofi_rdma_send_comm_t *)req->comm;
  const nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t *comm_rail = rdma_send_comm_get_rail(s_comm, 0);
  ...
  if ((rc != 0) && (rc != -FI_EAGAIN)) {
    NCCL_OFI_WARN("fi_write_inline failed; RC: %zd, Error: %s", rc, fi_strerror(-rc));
  }

  return rc;
}

static int post_rdma_write(nccl_net_ofi_rdma_req_t *req, nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t *comm_rail,
                           nccl_net_ofi_xfer_info_t *xfer_info) {
  rdma_req_send_data_t *send_data = get_send_data(req);
  assert(xfer_info->rail_id < send_data->buff_mr_handle->num_rails);
  const int rail_id = xfer_info->rail_id;
  fid_mr *rail_mr_handle = send_data->buff_mr_handle->mr[rail_id];
  void *desc = fi_mr_desc(rail_mr_handle);

  ssize_t rc = 0;
  /* Post RDMA write */
  rc = fi_writedata(comm_rail->local_ep, (void *)((uintptr_t)send_data->buff + xfer_info->offset), xfer_info->msg_size, desc, send_data->wdata,
                    comm_rail->remote_addr, send_data->remote_buff + xfer_info->offset, send_data->remote_mr_key[rail_id], req);
...
static int post_rdma_eager_send(nccl_net_ofi_rdma_req_t *req, nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t *comm_rail,
                                nccl_net_ofi_xfer_info_t *xfer_info) {
  rdma_req_send_data_t *send_data = get_send_data(req);
  assert(xfer_info->rail_id < send_data->buff_mr_handle->num_rails);
  const int rail_id = xfer_info->rail_id;
  ...
  ssize_t rc = 0;
  /* Post eager send */
  rc = fi_senddata(comm_rail->local_ep, (void *)(((uintptr_t)send_data->buff) + xfer_info->offset), xfer_info->msg_size, desc, send_data->wdata,
...
static int post_rx_buffer(nccl_net_ofi_rdma_req_t *req, nccl_net_ofi_ep_rail_t *ep_rail, bool set_fi_more) {
  const rdma_req_rx_buff_data_t *rx_buff_data = get_rx_buff_data(req);
  const nccl_ofi_freelist_elem_t *rx_buff_fl_elem = rx_buff_data->rx_buff_fl_elem;
  const auto *fl_mr_handle = (freelist_regmr_fn_handle_t *)rx_buff_fl_elem->mr_handle;
   ...
   * gets re-posted */
  const nccl_net_ofi_rdma_ep_t *ep = rx_buff_data->ep;
  nccl_ofi_freelist_entry_set_undefined(ep->rx_buff_fl, rx_buff_fl_elem->ptr);
  ...
  req->state = NCCL_OFI_RDMA_REQ_CREATED;
  const ssize_t rc = fi_recvmsg(ep_rail->ofi_ep, &msg, flags);
  if ((rc != 0) && (rc != -FI_EAGAIN)) {
    NCCL_OFI_WARN("Error posting rx buffer. RC: %zd, Error: %s", rc, fi_strerror(-rc));
  }
...
static int send_progress(nccl_net_ofi_rdma_req_t *req) {
  ...
  if (req->type == NCCL_OFI_RDMA_SEND) { // Post RDMA write
    ...
    if (send_data->eager) {
      ...
      nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t *comm_rail = rdma_send_comm_get_rail(s_comm, xfer_info->rail_id);
...
static int post_rdma_ctrl(nccl_net_ofi_rdma_req_t *req) {
  ...
  const nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_recv_comm_rail_t *comm_rail = nullptr;
  void *desc = nullptr;
  int rail_id = 0;

  const nccl_ofi_freelist_elem_t *ctrl_fl_elem = send_ctrl_data->ctrl_fl_elem;

  /* Unpack mr_handle */
  const auto *fl_handle = (freelist_regmr_fn_handle_t *)ctrl_fl_elem->mr_handle;
  const nccl_net_ofi_rdma_mr_handle_t *mr_handle = fl_handle->mr_handle;

  if (schedule != nullptr) {
    /* Use round robin schedule for ctrl message */
    const nccl_net_ofi_xfer_info_t *xfer_info = &schedule->rail_xfer_infos[0];
  ...
  const nccl_net_ofi_rdma_ctrl_msg_t *ctrl_msg = rdma_send_ctrl_get_msg(send_ctrl_data);

  const ssize_t rc = fi_send(comm_rail->local_ep, ctrl_msg, ctrl_msg_len, desc, comm_rail->remote_addr, req);

  if ((rc != 0) && (rc != -FI_EAGAIN)) {
    NCCL_OFI_WARN("Error posting RDMA ctrl request. RC: %zd, Error: %s", rc, fi_strerror(-rc));
  } else if (rc != -FI_EAGAIN) {
...
static int post_close_msg(nccl_net_ofi_rdma_req_t *req) {
  ...
  const nccl_ofi_freelist_elem_t *ctrl_fl_elem = send_close_data->ctrl_fl_elem;
  ...
  const nccl_net_ofi_rdma_close_msg_t *close_msg = rdma_send_close_get_msg(send_close_data);

  const ssize_t rc = fi_send(comm_rail->local_ep, close_msg, sizeof(nccl_net_ofi_rdma_close_msg_t), desc, comm_rail->remote_addr, req);

  if ((rc != 0) && (rc != -FI_EAGAIN)) {
    NCCL_OFI_WARN("Error posting RDMA close request. RC: %zd, Error: %s", rc, fi_strerror(-rc));
  }
...
static int post_eager_copy(nccl_net_ofi_rdma_req_t *req) {
  ...
  const rdma_req_recv_data_t *recv_data = get_recv_data(eager_copy_data->recv_req);

  /* Validate size of data */
  if (recv_data->dst_len < rx_buff_data->recv_len) {
    NCCL_OFI_TRACE(NCCL_NET, "Recv buffer (%zu) smaller than eager send size (%zu)", recv_data->dst_len, rx_buff_data->recv_len);
    rx_buff_data->recv_len = recv_data->dst_len;
  }

  // Get communicator rail information to xfer the req
  const nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_recv_comm_rail_t *comm_rail = nullptr;
  const int rx_rail_id = rx_buff_data->rail->rail_id;
  comm_rail = rdma_recv_comm_get_rail(r_comm, rx_rail_id);

  /* Unpack mr_handle */
  const auto *fl_handle = (freelist_regmr_fn_handle_t *)rx_buff_data->rx_buff_fl_elem->mr_handle;
  const nccl_net_ofi_rdma_mr_handle_t *rx_mr_handle = fl_handle->mr_handle;

  const nccl_net_ofi_rdma_mr_handle_t *dest_mr_handle = recv_data->dest_mr_handle;
  ...
  const uint64_t rx_key = fi_mr_key(rx_mr_handle->mr[rx_rail_id]);
  if (rx_key == FI_KEY_NOTAVAIL) {
    NCCL_OFI_WARN("Failed to get rx_key");
    return -EIO;
  }

  const ssize_t rc = fi_read(comm_rail->local_ep, recv_data->dst_buff, rx_buff_data->recv_len, desc, comm_rail->local_addr, (uint64_t)rx_buff, rx_key, req);
...
static int post_flush_req(nccl_net_ofi_rdma_req_t *req) {
  auto *r_comm = (nccl_net_ofi_rdma_recv_comm_t *)req->comm;
  const auto *ep = (nccl_net_ofi_rdma_ep_t *)r_comm->base.base.ep;
  nccl_net_ofi_rdma_recv_comm_t::nccl_net_ofi_rdma_flush_buffer_t *f_buff = &r_comm->flush_buff;
  ...
  for (int rail_id = 0; rail_id < ep->num_rails; rail_id++) {
    ...
    if (flush_data->mr_handle != nullptr) {
    ...
    const uint64_t host_buff_addr = (uint64_t)f_buff->host_buffer + (cpu_cache_line_size * rail_id);

    rc = fi_read(comm_rail->local_ep, (void *)host_buff_addr, f_buff->size, desc, comm_rail->local_addr, (uint64_t)(virt_addr_mr ? flush_data->data : nullptr),
                 cuda_key, req);
    if ((rc != 0) && (rc != -FI_EAGAIN)) {
      NCCL_OFI_WARN("Error posting flush request. RC: %zd, Error: %s", rc, fi_strerror(-rc));
      goto exit;
    }
  }
 ...
 *       	the application
 */
static int send(nccl_net_ofi_send_comm_t *send_comm, void *data, int size, int tag, nccl_net_ofi_mr_handle_t *mhandle, nccl_net_ofi_req_t **base_req) {
  int ret = 0;
  auto *s_comm = (nccl_net_ofi_rdma_send_comm_t *)send_comm;
  auto *mr_handle = (nccl_net_ofi_rdma_mr_handle_t *)mhandle;
  nccl_net_ofi_rdma_ep_t *ep = nullptr;
  nccl_net_ofi_rdma_req_t *req = nullptr;
...
retry:
  /* Retrieve entry from message buffer for msg_seq_num index */
  mb_res = nccl_ofi_msgbuff_retrieve(s_comm->msgbuff, msg_seq_num, &elem, &type, &msg_stat);
  if (mb_res == NCCL_OFI_MSGBUFF_SUCCESS) {
    if (type == NCCL_OFI_MSGBUFF_BUFF) [[likely]] {
      /*
      ...
      NCCL_OFI_WARN("Unexpected type of buffer retrieved from message buffer: %d", type);
      ret = -EINVAL;
      return onError();
    }
    ...
    NCCL_OFI_WARN("Message %hu has invalid status. res = %d and stat = %d", msg_seq_num, mb_res, msg_stat);
    ret = -EINVAL;
    return onError();
  }
  ...
  if (!polled_cq && !have_ctrl) [[unlikely]] {
    for (int rail_id = 0; rail_id != ep->num_control_rails; ++rail_id) {
  ...
  ret = alloc_rdma_send_req(s_comm, msg_seq_num, data, size, mr_handle, eager, &req);
  if (ret != 0) [[unlikely]] {
    return onError();
  }
...
static int send_close_deferred(nccl_net_ofi_send_comm_t *send_comm) {
  ...
  /* Deferred cleanup */
  nccl_ofi_deque_insert_back(s_comm_cleanup_list, &s_comm->cleanup_list_elem);
 ...
 *		NULL, on others
 */
static void prepare_send_connect_message(nccl_net_ofi_rdma_ep_t *ep, int dev_id, uint32_t local_comm_id, uint32_t remote_comm_id,
  ...
  /* Set libfabric endpoint names for each control rail */
  for (int rail_id = 0; rail_id != num_control_rails; ++rail_id) {
    memcpy(conn_msg->control_ep_names[rail_id].ep_name, ep->control_rails[rail_id].local_ep_name, ep->control_rails[rail_id].local_ep_name_len);
    conn_msg->control_ep_names[rail_id].ep_name_len = ep->control_rails[rail_id].local_ep_name_len;
  }

  /* Set libfabric endpoint names for each rail */
  for (int rail_id = 0; rail_id != num_rails; ++rail_id) {
    memcpy(conn_msg->ep_names[rail_id].ep_name, ep->rails[rail_id].local_ep_name, ep->rails[rail_id].local_ep_name_len);
    conn_msg->ep_names[rail_id].ep_name_len = ep->rails[rail_id].local_ep_name_len;
  }
}
 ...
 *		NULL, on error
 */
static nccl_net_ofi_rdma_send_comm_t *calloc_rdma_send_comm(int num_rails, int num_control_rails) {
  auto *s_comm = (nccl_net_ofi_rdma_send_comm_t *)calloc(1, sizeof(nccl_net_ofi_rdma_send_comm_t));
  if (!s_comm) [[unlikely]] {
    NCCL_OFI_WARN("Unable to allocate send communicator");
    goto error;
  }

  s_comm->rails = (nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t *)calloc(
      num_rails, sizeof(nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t));
  if (!s_comm->rails) [[unlikely]] {
    NCCL_OFI_WARN("Unable to allocate send communicator rails array");
    goto error;
  }

  s_comm->control_rails = (nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t *)calloc(
      num_control_rails, sizeof(nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t));
  if (!s_comm->control_rails) [[unlikely]] {
 ...
 *		non-zero, on error
 */
static int init_rx_buffers(nccl_net_ofi_rdma_ep_t *ep) {
  int ret = 0;
  nccl_net_ofi_ep_rail_t *rail = nullptr;

  ret = nccl_ofi_freelist_init(sizeof(nccl_net_ofi_rdma_req_t), ofi_nccl_rdma_min_posted_bounce_buffers(), 16, 0, &ep->rx_buff_reqs_fl);
  if (ret != 0) {
    NCCL_OFI_WARN("Failed to init rx_buff_reqs_fl");
    return ret;
  }

  ret = nccl_ofi_freelist_init_mr(ep->rx_buff_size, ofi_nccl_rdma_min_posted_bounce_buffers(), 16, 0, freelist_regmr_host_fn, freelist_deregmr_host_fn, ep,
                                  EAGER_RX_BUFFER_ALIGNMENT, &ep->rx_buff_fl);
  if (ret != 0) {
  ...
  for (int rail_id = 0; rail_id < ep->num_control_rails; ++rail_id) {
    ...
    nccl_net_ofi_mutex_init(&rail->rx_buff_mutex, nullptr);
  }
 ...
 *		non-zero, on error
 */
static int fini_rx_buffers(nccl_net_ofi_rdma_ep_t *ep) {
  int ret = 0;
  nccl_net_ofi_ep_rail_t *rail = nullptr;
...
static int get_mr_key(nccl_net_ofi_device_t *base_dev, void *mhandle, uint64_t *mr_key) {
  int ret = 0;
  const auto *mr_handle = (nccl_net_ofi_rdma_mr_handle_t *)mhandle;
 ...
 */
static int rma_write_impl(nccl_net_ofi_send_comm_t *send_comm, void *src, size_t size, void *desc, uint64_t dest, uint64_t mr_key, uint64_t flags,
                          nccl_net_ofi_req_t **base_req) {
  int ret = 0;
  auto *s_comm = (nccl_net_ofi_rdma_send_comm_t *)send_comm;
  nccl_net_ofi_rdma_req_t *req = nullptr;
...
static int rma_write(nccl_net_ofi_send_comm_t *send_comm, void *src, size_t size, void *mhandle, uint64_t dest, uint64_t mr_key,
                     nccl_net_ofi_req_t **base_req) {
  const auto *mr_handle = (nccl_net_ofi_rdma_mr_handle_t *)mhandle;
  fid_mr *rail_mr_handle = mr_handle->mr[0];
  void *desc = fi_mr_desc(rail_mr_handle);
  const uint64_t flags = 0;
...
static int rma_write_inline(nccl_net_ofi_send_comm_t *send_comm, void *src, size_t size, uint64_t dest, uint64_t mr_key, nccl_net_ofi_req_t **base_req) {
  void *desc = nullptr;
  const uint64_t flags = FI_INJECT;
  return rma_write_impl(send_comm, src, size, desc, dest, mr_key, flags, base_req);
}
 ...
 *
 */
static int create_send_comm(nccl_net_ofi_conn_handle_t *handle, nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_rdma_send_comm_t **s_comm) {
  int ret = 0;
  int comm_id = 0;
  fi_addr_t remote_addr = 0;
  nccl_net_ofi_rdma_send_comm_t *ret_s_comm = nullptr;
  const int num_rails = ep->num_rails;
  const int num_control_rails = ep->num_control_rails;
  const nccl_net_ofi_ep_rail_t *first_control_rail = rdma_endpoint_get_control_rail(ep, 0);
  nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t *first_comm_control_rail = nullptr;
  ...
  *s_comm = nullptr;
  ...
  if (ret_s_comm == nullptr) [[unlikely]] {
    NCCL_OFI_WARN("Couldn't allocate send comm object for dev %d", dev_id);
    return -ENOMEM;
  }

  ret = nccl_net_ofi_mutex_init(&ret_s_comm->ctrl_recv_lock, nullptr);
  if (ret != 0) {
  ...
  if (handle->comm_id >= device->num_comm_ids) [[unlikely]] {
    NCCL_OFI_WARN("Received an invalid communicator ID %" PRIu32 " for device %d", handle->comm_id, dev_id);
    ret = -EINVAL;
    goto error;
  }
  ...
  ret = fi_av_insert(first_control_rail->av, (void *)handle->ep_name, 1, &remote_addr, 0, nullptr);
  if (ret != 1) [[unlikely]] {
    NCCL_OFI_WARN("Unable to insert remote address into address vector for device %d. RC: %d", dev_id, ret);
    ret = -EINVAL;
    goto error;
  }
  ...
  ret = nccl_ofi_freelist_init(sizeof(nccl_net_ofi_rdma_req_t), 16, 16, NCCL_OFI_MAX_SEND_REQUESTS, &ret_s_comm->nccl_ofi_reqs_fl);
  if (ret != 0) [[unlikely]] {
    const int rail_id = 0;
    NCCL_OFI_WARN("Could not allocate NCCL OFI request free list for dev %d rail %d", dev_id, rail_id);
    goto error;
  }

  /* Allocate and initialize connect message */
  prepare_send_connect_message(ep, dev_id, ret_s_comm->local_comm_id, ret_s_comm->remote_comm_id, handle, &ret_s_comm->conn_msg);
 ...
 * 		NULL, others
 */
static nccl_net_ofi_rdma_req_t *prepare_send_conn_req(nccl_net_ofi_rdma_send_comm_t *s_comm) {
  nccl_net_ofi_rdma_req_t *req = nullptr;
 ...
 * 		NULL, others
 */
static nccl_net_ofi_rdma_req_t *prepare_recv_conn_resp_req(nccl_net_ofi_rdma_send_comm_t *s_comm) {
  nccl_net_ofi_rdma_req_t *req = nullptr;
 ...
 * 		others, on error
 */
static int post_send_conn(nccl_net_ofi_rdma_send_comm_t *s_comm, nccl_net_ofi_rdma_device_t *device, nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_rdma_req_t *req) {
  ssize_t rc = 0;
  const nccl_net_ofi_rdma_send_comm_t::nccl_net_ofi_rdma_send_comm_rail_t *comm_rail = rdma_send_comm_get_control_rail(s_comm, 0);
   ...
   * can be lifted.
   */
  rc = fi_send(comm_rail->local_ep, (void *)&s_comm->conn_msg, sizeof(nccl_ofi_rdma_connection_info_t), nullptr, comm_rail->remote_addr, req);
 ...
 * with only the first communicator rail being initialized, (b) post send
 ...
 */
static int connect(nccl_net_ofi_ep_t *base_ep, nccl_net_ofi_conn_handle_t *handle, nccl_net_ofi_send_comm_t **send_comm) {
  int ret = 0;
  ...
  *send_comm = nullptr;
  ...
  auto *req = (nccl_net_ofi_rdma_req_t *)comm_state->req;
  auto *s_comm = (nccl_net_ofi_rdma_send_comm_t *)comm_state->comm;

  /* Retrieve and validate devices */
  auto *device = (nccl_net_ofi_rdma_device_t *)base_ep->domain->device;
  assert(device != NULL);

  /* Connection establishment is not done yet */
  const nccl_ofi_comm_stage_t stage = comm_state->stage;
  if (stage == nccl_ofi_comm_stage_t::COMM_CONNECTED) {
    NCCL_OFI_WARN("Handle %p object already has an active send communicator (%p).", handle, s_comm);
    return -EINVAL;
  }
  ...
  switch (stage) {
  case nccl_ofi_comm_stage_t::COMM_CREATE_START:
    /* nccl_ofi_comm_stage_t::COMM_CREATE_START: Allocate data required for the
     * connect function */
    ...
    if (s_comm->conn_resp_req == nullptr) [[unlikely]] {
      send_comm_destroy(s_comm);
      return -EINVAL;
    }
  ...
  case nccl_ofi_comm_stage_t::COMM_SEND_CONN:
    ...
    /* nccl_ofi_comm_stage_t::COMM_SEND_CONN: Post a connect message to send peer connections */
    ...
    [[fallthrough]];
  case nccl_ofi_comm_stage_t::COMM_CONN_REQ_PENDING:
    /* nccl_ofi_comm_stage_t::COMM_CONN_REQ_PENDING: Wait until connect message
    ...
    req->free(req, false);
    comm_state->req = nullptr;
    req = nullptr;

    comm_state->stage = nccl_ofi_comm_stage_t::COMM_RECV_CONN;
    [[fallthrough]];
  case nccl_ofi_comm_stage_t::COMM_RECV_CONN:
    /* nccl_ofi_comm_stage_t::COMM_RECV_CONN: Receive connect response message from remote */

    assert(s_comm && s_comm->num_rails > 0);

    comm_state->stage = nccl_ofi_comm_stage_t::COMM_CONN_RESP_REQ_PENDING;
    [[fallthrough]];
  case nccl_ofi_comm_stage_t::COMM_CONN_RESP_REQ_PENDING:
...
static void ep_rail_release(nccl_net_ofi_ep_rail_t *rail, int dev_id, fid_cq *cq) {
  if (ofi_nccl_endpoint_per_communicator() != 0) {
  ...
  nccl_ofi_ofiutils_ep_release(rail->ofi_ep, rail->av, cq, dev_id);
  rail->ofi_ep = nullptr;
  rail->av = nullptr;
  rail->cq = nullptr;
}
...
static void release_rdma_ep_resources(nccl_net_ofi_rdma_ep_t *ep, int dev_id) {
  ...
  for (int rail_id = 0; rail_id != ep->num_control_rails; ++rail_id) {
    rail = rdma_endpoint_get_control_rail(ep, rail_id);
    ep_rail_release(rail, dev_id, nullptr);
  }
 ...
 * 		-EINVAL, others
 */
static int set_local_address(fid_ep *ep, nccl_net_ofi_ep_rail_t *rail) {
  int res = 0;
  rail->local_ep_name_len = sizeof(rail->local_ep_name);

  res = fi_getname(&ep->fid, (void *)rail->local_ep_name, &rail->local_ep_name_len);
  if (res == -FI_ETOOSMALL) {
    NCCL_OFI_WARN("Endpoint's address length (%zu) is larger than supplied buffer length (%d)", rail->local_ep_name_len, MAX_EP_ADDR);
    return -EINVAL;
  } else if (res != 0) {
...
static int ep_rail_init(nccl_net_ofi_rdma_ep_t *ep, int dev_id, int rail_id, nccl_net_ofi_rdma_device_t::nccl_net_ofi_rdma_device_rail_t *dev_rail,
                        nccl_net_ofi_rdma_domain_t::nccl_net_ofi_rdma_domain_rail_t *domain_rail, nccl_net_ofi_ep_rail_t *ep_rail, uint32_t tclass) {
  int ret = 0;
  fi_info *rail_info = dev_rail->info;

  ep_rail->domain = domain_rail->domain;
  if (ep_rail->cq == nullptr) {
  ...
  ret = nccl_ofi_ofiutils_init_connection(dev_rail->info, ep_rail->domain, &ep_rail->ofi_ep, &ep_rail->av, &ep_rail->cq);
  if (tclass != FI_TC_UNSPEC) {
 ...
 */
static int init_rail_ofi_resources(nccl_net_ofi_rdma_device_t *device, nccl_net_ofi_rdma_domain_t *domain, nccl_net_ofi_rdma_ep_t *ep) {
  int ret = 0;
  const int dev_id = device->base.dev_id;
  nccl_net_ofi_rdma_device_t::nccl_net_ofi_rdma_device_rail_t *rail_dev = nullptr;
  nccl_net_ofi_rdma_domain_t::nccl_net_ofi_rdma_domain_rail_t *domain_rail = nullptr;
  nccl_net_ofi_ep_rail_t *rail = nullptr;
  nccl_net_ofi_ep_rail_t *control_rail = nullptr;
  const uint32_t tc = (ofi_nccl_use_low_lat_tc() == 0) ? FI_TC_UNSPEC : FI_TC_LOW_LATENCY;
...
static int nccl_net_ofi_rdma_endpoint_release(nccl_net_ofi_ep_t *base_ep) {
   ...
   * the endpoint per communicator code, we need to use a
...
static int init_max_write_inline_size_if_not_initialized(nccl_net_ofi_rdma_device_t *device, nccl_net_ofi_rdma_ep_t *ep) {
  int ret = 0;
  if (is_max_write_inline_size_initialized == false) {
    ...
    ret = get_inject_rma_size_opt(ep->rails[0].ofi_ep, &max_write_inline_size);
    if (ret == 0) {
      ...
      NCCL_OFI_WARN("Failed to retrieve maximum write size");
    }
  }
  return ret;
}
...
static int nccl_net_ofi_rdma_domain_create_endpoint(nccl_net_ofi_domain_t *base_domain, nccl_net_ofi_ep_t **base_ep) {
  int ret = 0;
  nccl_net_ofi_rdma_ep_t *ep = nullptr;
  nccl_net_ofi_rdma_domain_t *domain = nullptr;
  nccl_net_ofi_rdma_device_t *device = nullptr;
  ...
  ep->rails = (nccl_net_ofi_ep_rail_t *)calloc(ep->num_rails, sizeof(nccl_net_ofi_ep_rail_t));
  if (!ep->rails) {
  ...
  ep->rx_buff_size = std::max({sizeof(nccl_net_ofi_rdma_ctrl_msg_t), eager_max_size, sizeof(nccl_ofi_rdma_connection_info_t)});
...
static int nccl_net_ofi_rdma_domain_free(nccl_net_ofi_domain_t *base_domain) {
  int ret = 0;
  auto *domain = (nccl_net_ofi_rdma_domain_t *)base_domain;

  for (int i = 0; i < domain->num_rails; ++i) {
    if (domain->domain_rails[i].cq != nullptr) {
      fi_close(&domain->domain_rails[i].cq->fid);
      domain->domain_rails[i].cq = nullptr;
    }
...
static nccl_net_ofi_domain_t *nccl_net_ofi_rdma_device_create_domain(nccl_net_ofi_device_t *base_dev) {
  ...
  if (ofi_nccl_endpoint_per_communicator() != 0) {
    ...
    if (domain->ep_addr_list == nullptr) {
  ...
  domain->domain_rails = (nccl_net_ofi_rdma_domain_t::nccl_net_ofi_rdma_domain_rail_t *)calloc(
      domain->num_rails, sizeof(nccl_net_ofi_rdma_domain_t::nccl_net_ofi_rdma_domain_rail_t));
  if (domain->domain_rails == nullptr) {
    NCCL_OFI_WARN("Unable to allocate rdma rails");
    ret = -ENOMEM;
    goto error;
  }
  ...
  for (int i = 0; i < domain->num_rails; i++) {
    const nccl_net_ofi_rdma_device_t::nccl_net_ofi_rdma_device_rail_t *device_rail = rdma_device_get_rail(device, i);
    nccl_net_ofi_rdma_domain_t::nccl_net_ofi_rdma_domain_rail_t *domain_rail = rdma_domain_get_rail(domain, i);

    ret = fi_domain(device_rail->fabric, device_rail->info, &domain_rail->domain, nullptr);
    if (ret != 0) [[unlikely]] {
    ...
    if (ofi_nccl_endpoint_per_communicator() != 0) {
      ...
      ret = fi_cq_open(domain_rail->domain, &cq_attr, &domain_rail->cq, nullptr);
      if (ret != 0) [[unlikely]] {
 ...
 */
static int init_device_rail_ofi_resources(nccl_net_ofi_rdma_device_t *device, nccl_net_ofi_rdma_device_t::nccl_net_ofi_rdma_device_rail_t *rail_dev) {
  int ret = 0;
...
static int device_prepare_for_connection(nccl_net_ofi_rdma_device_t *device) {
  int ret = 0;
  nccl_net_ofi_rdma_device_t::nccl_net_ofi_rdma_device_rail_t *begin = device->device_rails;
  const nccl_net_ofi_rdma_device_t::nccl_net_ofi_rdma_device_rail_t *end = device->device_rails + device->num_rails;
...
static void release_device_ofi_resources(nccl_net_ofi_rdma_device_t *device) {
  const nccl_net_ofi_rdma_device_t::nccl_net_ofi_rdma_device_rail_t *begin = device->device_rails;
  const nccl_net_ofi_rdma_device_t::nccl_net_ofi_rdma_device_rail_t *end = device->device_rails + device->num_rails;
 ...
 *		NULL, on others
 */
static nccl_net_ofi_rdma_device_t::nccl_net_ofi_rdma_device_rail_t *create_device_rail_array(fi_info *info_list, int num_infos) {
  /* Allocate NIC info array */
  auto *device_rails =
      (nccl_net_ofi_rdma_device_t::nccl_net_ofi_rdma_device_rail_t *)calloc(num_infos, sizeof(nccl_net_ofi_rdma_device_t::nccl_net_ofi_rdma_device_rail_t));
  ...
  for (int i = 0; i < num_infos; i++) {
    ...
    /* Libfabric documentation is not clear if next is
 ...
 */
static int nccl_net_ofi_rdma_device_release(nccl_net_ofi_device_t *base_device) {
  auto *device = (nccl_net_ofi_rdma_device_t *)base_device;
  int ret = 0, first_error = 0;

  if (device == nullptr) {
    return 0;
  }

  const unsigned num_domains = HASH_COUNT(device->base.domain_table);
  if (num_domains > 0) {
    NCCL_OFI_INFO(NCCL_NET, "%u domains still active at close", num_domains);
  }

  if (device->device_rails != nullptr) {
  ...
  if (device->scheduler) {
    ...
    if (ret != 0) {
      NCCL_OFI_WARN("Cleanup of device failed, scheduler_fini returned %s", strerror(-ret));
      if (first_error == 0) {
        first_error = ret;
      }
    }
  ...
  if (device->comm_idpool) {
    ...
    device->comm_idpool = nullptr;
  }

  ret = nccl_net_ofi_device_fini(base_device);
  if (ret != 0) {
    NCCL_OFI_WARN("Cleanup of device failed, device_fini returned %s", strerror(-ret));
    if (first_error == 0) {
      first_error = ret;
    }
  }
 ...
 */
static nccl_net_ofi_rdma_device_t *nccl_net_ofi_rdma_device_create(nccl_net_ofi_plugin_t *plugin, int dev_id, fi_info *info_list, nccl_ofi_topo_t *topo,
                                                                   size_t min_strip_size) {
  int ret = 0;
  int length = 0;
  auto *device = (nccl_net_ofi_rdma_device_t *)calloc(1, sizeof(nccl_net_ofi_rdma_device_t));
  if (device == nullptr) {
    NCCL_OFI_WARN("Unable to allocate device %i", dev_id);
    return nullptr;
  }

  ret = nccl_net_ofi_device_init(&device->base, plugin, dev_id, info_list);
  if (ret != 0) {
  ...
  if (topo->max_group_size != length) {
    NCCL_OFI_WARN("Wrong number of NICs for device %i. Expected %i but got %i", dev_id, topo->max_group_size, length);
    goto error;
  }
  ...
  if (ret != 0) {
    NCCL_OFI_WARN("preparing for connection failed: %s", strerror(-ret));
    goto error;
  }

  /* Create array of comms. */
  /* TODO make this array expandable */
  device->comms = (nccl_net_ofi_comm_t **)calloc(NCCL_OFI_RDMA_MAX_COMMS, sizeof(nccl_net_ofi_comm_t *));
  if (!device->comms) {
  ...
  if (device->comm_idpool == nullptr) [[unlikely]] {
...
static void get_hints(fi_info *hints) {
  hints->caps = 0;
...
static int nccl_net_ofi_rdma_plugin_fini(nccl_net_ofi_plugin_t *plugin) {
  ...
  auto *rdma_plugin = (nccl_net_ofi_rdma_plugin_t *)plugin;

  if (rdma_plugin->topo != nullptr) {
  ...
  if (ret != 0) {
    NCCL_OFI_WARN("Destructing base plugin failed: %s", strerror(-ret));
    if (last_error == 0) {
      last_error = ret;
    }
  }
...
static int nccl_net_ofi_rdma_plugin_complete_init(nccl_net_ofi_plugin_t *plugin) {
  auto *rdma_plugin = (nccl_net_ofi_rdma_plugin_t *)plugin;
  ...
  for (size_t dev_id = 0; dev_id != rdma_plugin->base.p_num_devs; ++dev_id) {
    ...
    nccl_net_ofi_rdma_device_t *device =
        nccl_net_ofi_rdma_device_create(&rdma_plugin->base, (int)dev_id, info_list, rdma_plugin->topo, ofi_nccl_min_stripe_size());
    if (device == nullptr) {
...
static int nccl_net_ofi_rdma_plugin_create(size_t num_devices, nccl_ofi_topo_t *topo, nccl_net_ofi_rdma_plugin_t **plugin_p) {
  int ret = 0;
  nccl_net_ofi_rdma_plugin_t *plugin = nullptr;
  ...
  if (ret != 0) {
    NCCL_OFI_WARN("Initializing base plugin failed: %s", strerror(-ret));
    free(plugin);
    return ret;
  }
...
int nccl_net_ofi_rdma_init(const char *provider_filter, nccl_net_ofi_plugin_t **plugin_p, bool *found_multiple_rails) {
  int ret = 0;
  int num_devs = 0;
  ...
  *found_multiple_rails = false;
  ...
  ret = nccl_ofi_ofiutils_get_providers(provider_filter, api_version, hints, &provider_list, &num_providers);
  if (ret == 0) {
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Using Libfabric %u.%u API, with %s support", FI_MAJOR(api_version), FI_MINOR(api_version),
     ...
     * CUDA
     */
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Using Libfabric 1.18 API, with GPUDirect RDMA support");
  ...
  if (topo->max_group_size > MAX_NUM_RAILS) {
    NCCL_OFI_WARN("Unexpected topo group size of %d (maximum %d)", topo->max_group_size, MAX_NUM_RAILS);
    ret = -EINVAL;
    goto error;
  }
