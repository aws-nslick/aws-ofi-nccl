							...
							nccl_ofi_rdma_connection_info_t *conn_msg)
{
	...
	if (ofi_nccl_endpoint_per_communicator() != 0)
	{
		std::string_view remote_rail0_ep_name{conn_msg->ep_names[0].ep_name, conn_msg->ep_names[0].ep_name_len};
		auto endpoint = domain->address_associations->find_not_connected(remote_rail0_ep_name);
		if (not endpoint.has_value()) {
			ret = domain->base.create_endpoint(&domain->base, &r_comm->base.base.ep);
			assert(ret == 0);
			((nccl_net_ofi_rdma_ep_t *)r_comm->base.base.ep)->is_endpoint_per_communicator_ep = true;
		} else {
			r_comm->base.base.ep = endpoint.value();
		}
		domain->address_associations->make_association(remote_rail0_ep_name, r_comm->base.base.ep);
	} else {
