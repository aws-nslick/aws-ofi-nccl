...
static inline rdma_req_rx_buff_data_t *get_rx_buff_data(nccl_net_ofi_rdma_req_t *req) {
  assert((req->type == NCCL_OFI_RDMA_CTRL_RX_BUFF) || (req->type == NCCL_OFI_RDMA_EAGER_RX_BUFF));
...
static inline int handle_rx_buff_recv(nccl_net_ofi_rdma_device_t *device, int rail_id, struct fi_cq_data_entry *cq_entry, nccl_net_ofi_rdma_req_t *rx_buff_req,
  ...
  if (OFI_UNLIKELY((eager && (rx_buff_req->type != NCCL_OFI_RDMA_EAGER_RX_BUFF)) || ((!eager) && (rx_buff_req->type != NCCL_OFI_RDMA_CTRL_RX_BUFF)))) {
...
static inline nccl_net_ofi_rdma_req_t *eager_rx_buff_req_alloc(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_ep_rail_t *rail) {
  ...
  nccl_ofi_freelist_elem_t *rx_buff_fl_elem = nccl_ofi_freelist_entry_alloc(ep->eager_rx_buff_fl);
  if (!rx_buff_fl_elem) {
...
static inline nccl_net_ofi_rdma_req_t *ctrl_rx_buff_req_alloc(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_ep_rail_t *rail) {
  nccl_net_ofi_rdma_req_t *req = allocate_req(ep->rx_buff_reqs_fl);
  if (!req)
    return NULL;
  ...
  nccl_ofi_freelist_elem_t *rx_buff_fl_elem = nccl_ofi_freelist_entry_alloc(ep->ctrl_rx_buff_fl);
  if (!rx_buff_fl_elem) {
    NCCL_OFI_WARN("Failed to allocate rx_buff_fl_elem");
    req->free(req, false);
    return NULL;
...
static inline int post_rx_buffs_on_rail(nccl_net_ofi_rdma_ep_t *ep, nccl_net_ofi_ep_rail_t *rail) {
  ...
  for (size_t i = 0; i < buffers_needed; ++i) {
    ...
    nccl_net_ofi_rdma_req_t *req = rail->rx_buff_req_alloc(ep, rail);
    if (!req) {
...
static int test(nccl_net_ofi_req_t *base_req, int *done, int *size) {
  ...
  *done = 0;
   ...
   * completed */
  if (req->state != NCCL_OFI_RDMA_REQ_COMPLETED && OFI_LIKELY(req->state != NCCL_OFI_RDMA_REQ_ERROR)) {
...
static inline int insert_rdma_recv_req_into_msgbuff(nccl_net_ofi_rdma_recv_comm_t *r_comm, bool eager, nccl_net_ofi_rdma_req_t **ret_req) {
  ...
  if (eager) {
    ...
    if (OFI_UNLIKELY((mb_res == NCCL_OFI_MSGBUFF_INVALID_IDX) && (msg_stat == NCCL_OFI_MSGBUFF_INPROGRESS))) {
...
static int post_rx_buffer(nccl_net_ofi_rdma_req_t *req, nccl_net_ofi_ep_rail_t *ep_rail, bool set_fi_more) {
   ...
   * gets re-posted */
  nccl_net_ofi_rdma_ep_t *ep = rx_buff_data->ep;
  nccl_ofi_freelist_t *fl = (req->type == NCCL_OFI_RDMA_EAGER_RX_BUFF ? ep->eager_rx_buff_fl : ep->ctrl_rx_buff_fl);
...
static int send_progress(nccl_net_ofi_rdma_req_t *req) {
  ...
  } else if (req->type == NCCL_OFI_RDMA_CTRL_RX_BUFF || req->type == NCCL_OFI_RDMA_EAGER_RX_BUFF) { // Post rx Buffer
    rdma_req_rx_buff_data_t *rx_buff_data = get_rx_buff_data(req);
...
static ssize_t send_ctrl_post(nccl_net_ofi_rdma_recv_comm_t *r_comm, nccl_ofi_freelist_elem_t *ctrl_fl_elem, int rail_id, size_t size,
  ...
  ssize_t rc = fi_send(comm_rail->local_ep, ctrl_fl_elem->ptr, size, desc, comm_rail->remote_addr, req);
  if ((rc != 0) && (rc != -FI_EAGAIN)) {
    NCCL_OFI_WARN("Error posting RDMA %s request. RC: %zd, Error: %s", nccl_net_ofi_req_str(req), rc, fi_strerror(-rc));
  }
  return rc;
}
...
static int post_close_msg(nccl_net_ofi_rdma_req_t *req) {
  ...
  ssize_t rc = send_ctrl_post(r_comm, ctrl_fl_elem, rail_id, sizeof(nccl_net_ofi_rdma_close_msg_t), req);
...
static inline int init_rx_buffers(nccl_net_ofi_rdma_ep_t *ep) {
  ...
  ret = nccl_ofi_freelist_init_mr(ep->ctrl_rx_buff_size, ofi_nccl_rdma_min_posted_bounce_buffers(), 16, 0, freelist_regmr_host_fn, freelist_deregmr_host_fn, ep,
                                  1, &ep->ctrl_rx_buff_fl);
  if (ret != 0) {
  ...
  ret = nccl_ofi_freelist_init_mr(ep->eager_rx_buff_size, ofi_nccl_rdma_min_posted_bounce_buffers(), 16, 0, freelist_regmr_host_fn, freelist_deregmr_host_fn,
...
static int nccl_net_ofi_rdma_domain_create_endpoint(nccl_net_ofi_domain_t *base_domain, nccl_net_ofi_ep_t **base_ep) {
  ...
  ep->ctrl_rx_buff_size =
      NCCL_OFI_MAX(sizeof(nccl_net_ofi_rdma_ctrl_msg_t), NCCL_OFI_MAX(sizeof(nccl_ofi_rdma_connection_info_t), sizeof(nccl_net_ofi_rdma_close_msg_t)));
...
static nccl_net_ofi_rdma_device_t *nccl_net_ofi_rdma_device_create(nccl_net_ofi_plugin_t *plugin, int dev_id, struct fi_info *info_list, nccl_ofi_topo_t *topo,
  ...
  if (target_length != 0) {
    ...
    if (length > target_length) {
      ...
      NCCL_OFI_WARN("Number of forced rails (%d) not a multiple of numer of rails (%d)", (int)target_length, length);
      goto error;
    ...
    NCCL_OFI_INFO(NCCL_NET, "Created device with %d rails (originally found %d rails)", length, original_length);
  } else {
