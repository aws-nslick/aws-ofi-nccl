...
void nccl_ofi_mr_cache_finalize(nccl_ofi_mr_cache_t *cache) {
  ...
  NCCL_OFI_INFO(NCCL_NET, "MR cache %d hits %d misses", cache->hit_count, cache->miss_count);
...
void *nccl_ofi_mr_cache_lookup_entry(nccl_ofi_mr_cache_t *cache, nccl_ofi_mr_ckey_ref ckey) {
  ...
  for (size_t slot = 0;; slot++) {
    ...
    if (slot == cache->used || page_addr < cache->slots[slot]->addr) {
      ...
      /* cache hit */
      cache->hit_count++;
      NCCL_OFI_TRACE(NCCL_NET, "Found MR handle %p for %ld(%s) in cache slot %zu", cache->slots[slot]->handle, nccl_ofi_mr_ckey_baseaddr(ckey),
                     nccl_ofi_mr_ckey_type_str(ckey), slot);
...
int nccl_ofi_mr_cache_insert_entry(nccl_ofi_mr_cache_t *cache, nccl_ofi_mr_ckey_ref ckey, void *handle) {
  ...
  for (size_t slot = 0;; slot++) {
    ...
    if (slot == cache->used || page_addr < cache->slots[slot]->addr) {
      ...
      cache->slots[slot] = (nccl_ofi_reg_entry_t *)calloc(1, sizeof(nccl_ofi_reg_entry_t));
      if (!cache->slots[slot]) {
      ...
      cache->used++;
      NCCL_OFI_TRACE(NCCL_NET, "Inserted MR handle %p for %ld(%s) in cache slot %zu", handle, nccl_ofi_mr_ckey_baseaddr(ckey), nccl_ofi_mr_ckey_type_str(ckey),
                     slot);
      goto out;
    } else if ((page_addr >= cache->slots[slot]->addr) &&
               ((page_addr - cache->slots[slot]->addr) / cache->system_page_size + pages) <= cache->slots[slot]->pages) {
      /* cache hit */
      NCCL_OFI_WARN("Entry already exists for input (%s) base %lu size %zu", nccl_ofi_mr_ckey_type_str(ckey), nccl_ofi_mr_ckey_baseaddr(ckey),
...
int nccl_ofi_mr_cache_del_entry(nccl_ofi_mr_cache_t *cache, void *handle) {
  ...
  if (--cache->slots[slot]->refcnt) {
    NCCL_OFI_TRACE(NCCL_NET, "Decremented refcnt for MR handle %p in cache slot %d", handle, slot);
    goto out;
  }

  /* Free this entry and defrag cache */
  free(cache->slots[slot]);
  memmove(cache->slots + slot, cache->slots + slot + 1, (cache->used - slot - 1) * sizeof(nccl_ofi_reg_entry_t *));
  --cache->used;

  NCCL_OFI_TRACE(NCCL_NET, "Removed MR handle %p in cache slot %d", handle, slot);
