...
nccl_ofi_msgbuff_t *nccl_ofi_msgbuff_init(uint16_t max_inprogress, uint16_t bit_width) {
  ...
  msgbuff->buff = (nccl_ofi_msgbuff_elem_t *)malloc(sizeof(nccl_ofi_msgbuff_elem_t) * max_inprogress);
  if (!msgbuff->buff) {
 ...
 */
static nccl_ofi_msgbuff_status_t nccl_ofi_msgbuff_get_idx_status(const nccl_ofi_msgbuff_t *msgbuff, uint16_t msg_index) {
  /* Test for INPROGRESS: index is between msg_last_incomplete (inclusive) and msg_next
   * (exclusive) */
  if (distance(msgbuff, msg_index, msgbuff->msg_last_incomplete) < distance(msgbuff, msgbuff->msg_next, msgbuff->msg_last_incomplete)) {
   ...
   * wraparound */
  if (msg_index != msgbuff->msg_last_incomplete && distance(msgbuff, msgbuff->msg_last_incomplete, msg_index) <= msgbuff->max_inprogress) {
    return NCCL_OFI_MSGBUFF_COMPLETED;
  }

  /* Test for NOTSTARTED: index is >= msg_next and there is room in the buffer */
  if (distance(msgbuff, msg_index, msgbuff->msg_next) < distance(msgbuff, msgbuff->max_inprogress, nccl_ofi_msgbuff_num_inflight(msgbuff))) {
