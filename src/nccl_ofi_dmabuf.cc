...
int nccl_ofi_dmabuf_viable() {
  ...
  if (!nccl_net_ofi_cuda_have_dma_buf_attr()) {
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Will not attempt to use DMA-BUF, CU_DEVICE_ATTRIBUTE_DMA_BUF_SUPPORTED was false.");
    return false;
  }
#endif
   ...
   * FI_MR_DMABUF flag. */
  NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Attempting to use DMA-BUF capable providers. set OFI_NCCL_DISABLE_DMABUF=1 to disable");
  return true;
}
