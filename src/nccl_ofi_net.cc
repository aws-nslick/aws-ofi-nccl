...
int nccl_net_ofi_create_plugin(nccl_net_ofi_plugin_t **plugin_p) {
  ...
  if (nccl_ofi_selected_protocol != nullptr) {
    ...
    NCCL_OFI_INFO(NCCL_INIT | NCCL_NET, "Using transport protocol %s", nccl_ofi_selected_protocol);
  }
  ...
  NCCL_OFI_INFO(NCCL_INIT | NCCL_NET, "Creating one domain per %s", plugin->domain_per_thread ? "thread" : "process");
  ...
  NCCL_OFI_INFO(NCCL_NET | NCCL_INIT, "Support for global registrations: %s", (properties.regIsGlobal == 0) ? "false" : "true");
  NCCL_OFI_INFO(NCCL_NET | NCCL_INIT, "Support for DMA-BUF registrations: %s", (properties.dmabuf_support == 0) ? "false" : "true");
...
static int get_device_pci_path(fid_nic *nic_info, char **path) {
  ...
  ret = asprintf(&device_path, "/sys/class/pci_bus/%04x:%02x/../../%04x:%02x:%02x.%01x", pci->domain_id, pci->bus_id, pci->domain_id, pci->bus_id,
...
int nccl_net_ofi_info_properties(nccl_net_ofi_plugin_t *plugin, fi_info *nic_prov, int dev_id, int num_devices, nccl_ofi_properties_t *props) {
  ...
  if (nic_dup_conns > 1) {
    ...
    NCCL_OFI_TRACE(NCCL_INIT, "Returning synthetic PCI path for device %d of %s", dev_id, props->pci_path);

    snprintf(props->name, FI_NAME_MAX + 2, "%s-%x", nic_info->device_attr->name, dev_id);
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Adjusted dev %d device name to %s", dev_id, props->name);
#else
  ...
  props->dmabuf_support = ((nic_prov->caps & FI_HMEM) != 0) && FI_VERSION_GE(nic_prov->fabric_attr->api_version, FI_VERSION(1, 20)) && nccl_ofi_dmabuf_viable();
...
int nccl_net_ofi_query_provider_capabilities(const fi_info *selected_provider, unsigned int num_providers) {
  ...
  if (selected_provider->domain_attr->mr_mode & FI_MR_LOCAL) {
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Provider %s requires registration of local memory buffers", selected_provider->fabric_attr->prov_name);
    local_mr = true;
  } else {
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Provider %s does not require registration of local memory buffers", selected_provider->fabric_attr->prov_name);
    local_mr = false;
  }

  /* Check if provider uses remote virtual addressing */
  if (selected_provider->domain_attr->mr_mode & FI_MR_VIRT_ADDR) {
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Provider %s uses remote virtual addressing", selected_provider->fabric_attr->prov_name);
    virt_addr_mr = true;
  } else {
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Provider %s does not use remote virtual addressing", selected_provider->fabric_attr->prov_name);
    virt_addr_mr = false;
  }

  /* Check if provider uses endpoint memory registration */
  if (selected_provider->domain_attr->mr_mode & FI_MR_ENDPOINT) {
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Provider %s requires endpoint memory registration", selected_provider->fabric_attr->prov_name);
    endpoint_mr = true;
  } else {
    NCCL_OFI_TRACE(NCCL_INIT | NCCL_NET, "Provider %s does not require endpoint memory registration", selected_provider->fabric_attr->prov_name);
    endpoint_mr = false;
  }
...
static int nccl_net_ofi_plugin_assign_device(nccl_net_ofi_plugin_t *plugin, size_t device_index, nccl_net_ofi_device_t *device) {
...
static size_t nccl_net_ofi_plugin_get_num_devices(nccl_net_ofi_plugin_t *plugin) { return plugin->p_num_devs; }
...
/*
 * implementation of retrieving a domain from a device.  This code
...
nccl_net_ofi_domain_t static *nccl_net_ofi_device_get_domain_impl(nccl_net_ofi_device_t *device) {
  ...
  if (domain == nullptr) {
    ...
    HASH_ADD(hh, device->domain_table, creating_thread_id, sizeof(domain->creating_thread_id), domain);

    NCCL_OFI_TRACE(NCCL_NET, "Domain %p for device #%d (%s) is created", domain, device->dev_id, device->name);
  }
...
static int nccl_net_ofi_domain_get_ep(nccl_net_ofi_domain_t *domain, nccl_net_ofi_ep_t **ep_p) {
  ...
  if (ep == nullptr) {
    ...
    HASH_ADD(hh, domain->endpoint_table, creating_thread_id, sizeof(ep->creating_thread_id), ep);

    NCCL_OFI_TRACE(NCCL_NET, "Eendpoint %p for domain %p is created", ep, domain);
  }
...
int nccl_net_ofi_domain_init(nccl_net_ofi_device_t *device, nccl_net_ofi_domain_t *domain) {
  ...
  if (device->need_mr_rkey_pool) {
    ...
    if (shift > (size_t_bits - 1)) {
      NCCL_OFI_WARN("Provided mr keypool size of %lu must be less than %zu", ofi_nccl_mr_key_size(), size_t_bits);
      return -EINVAL;
    }
  ...
  if (ret != 0) {
    NCCL_OFI_WARN("Creating MR id pool failed: %s", strerror(-ret));
    return -ret;
  }
...
int get_inject_rma_size_opt(fid_ep *ofi_ep, size_t *max_write_inline_size) {
  ...
  if (ret != 0 && ret != -FI_ENOPROTOOPT) {
    NCCL_OFI_WARN("Retrieving option endpoint FI_OPT_INJECT_RMA_SIZE failed. RC: %d. Error: %s", ret, fi_strerror(-ret));
  }
  return ret;
#else
